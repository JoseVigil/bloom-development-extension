...
# Script movido a builds/windows/ — dos niveles arriba es la raíz
# =========================================
...
# Limpiar cache (relativo a la raíz del repo)
$dirsToClean = @(
    "$REPO_ROOT\build",
    "$REPO_ROOT\dist",
...
$platformDir = if ($arch -eq "AMD64" -or $arch -eq "x86_64") { "win64" } else { "win32" }

$possiblePaths = @(
    "$REPO_ROOT\installer\native\bin\$platformDir\brain\brain.exe",
    "$REPO_ROOT\installer\brain\dist\brain\brain.exe"
)
