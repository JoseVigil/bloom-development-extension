...
class CiCdStrategy:
    ...
    def analyze(self) -> Dict:
        ...
        if gh_dir.exists():
            config_files.extend([str(p.relative_to(self.root)) for p in gh_dir.glob("*.yml")])
        if (self.root / ".gitlab-ci.yml").exists():
            config_files.append('.gitlab-ci.yml')
        if (self.root / "Jenkinsfile").exists():
            config_files.append('Jenkinsfile')
        return {
            'config_files': config_files,
            'raw_data': {'pipelines': data}
        }
