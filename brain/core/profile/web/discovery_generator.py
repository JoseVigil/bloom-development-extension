...
def generate_discovery_page(target_ext_dir: Path, profile_data: Dict[str, Any]) -> None:
    ...
    """
    logger.info(f"🔧 Desplegando assets estáticos de discovery para: {profile_data.get('alias')}")
    ...
    logger.info(f"  ✅ Assets estáticos desplegados en: {discovery_dir}")
    logger.info(f"  ℹ️  Los archivos de configuración serán generados por Sentinel durante el launch")
...
#
# - _generate_synapse_config()  → Now in ignition_identity.go::prepareSessionFiles()
# - _generate_launch_id()       → Now in ignition_identity.go::generateLogicalLaunchID()
# - update_discovery_config()   → Config updates via Sentinel's override system
#
