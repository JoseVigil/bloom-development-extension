"""OpenAI keys list command."""
import typer
from brain.cli.base import BaseCommand, CommandMetadata
from brain.cli.categories import CommandCategory


class OpenAIKeysListCommand(BaseCommand):
    def metadata(self) -> CommandMetadata:
        return CommandMetadata(
            name="keys-list",
            category=CommandCategory.OPENAI,
            version="1.0.0",
            description="List all OpenAI API keys",
            examples=["brain openai keys-list", "brain openai keys-list --json"]
        )
    
    def register(self, app: typer.Typer):
        @app.command("keys-list")
        def list_keys(ctx: typer.Context):
            """List all registered OpenAI API keys."""
            gc = ctx.obj
            if gc is None:
            ...
            try:
                ...
                if not keys:
                    ...
                    if gc.json_mode:
                        ...
                        typer.echo("ℹ️  No OpenAI keys configured")
                    return
                ...
                if gc.json_mode:
                    ...
                    typer.echo(f"📊 OpenAI API Keys ({len(keys_data)} total)\n")
