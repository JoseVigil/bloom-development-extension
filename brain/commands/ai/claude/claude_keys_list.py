"""Claude keys list command."""
import typer
from brain.cli.base import BaseCommand, CommandMetadata
from brain.cli.categories import CommandCategory


class ClaudeKeysListCommand(BaseCommand):
    def metadata(self) -> CommandMetadata:
        return CommandMetadata(
            name="keys-list",
            category=CommandCategory.CLAUDE,
            version="1.0.0",
            description="List all Claude API keys",
            examples=["brain claude keys-list", "brain claude keys-list --json"]
        )
    
    def register(self, app: typer.Typer):
        @app.command("keys-list")
        def list_keys(ctx: typer.Context):
            """List all registered Claude API keys."""
            gc = ctx.obj
            if gc is None:
            ...
            try:
                ...
                if not keys:
                    ...
                    if gc.json_mode:
                        ...
                        typer.echo("ℹ️  No Claude keys configured")
                    return
                ...
                if gc.json_mode:
                    ...
                    typer.echo(f"📊 Claude API Keys ({len(keys_data)} total)\n")
