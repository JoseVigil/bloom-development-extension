...
class ProfilesLaunchCommand(BaseCommand):
    ...
    def metadata(self) -> CommandMetadata:
        return CommandMetadata(
            name="launch",
            category=CommandCategory.PROFILE,
            version="2.6.0",  # Version bump: SynapseSimulator mode support
            ...
            examples=[
                ...
                "brain profile launch abc12345 --spec spec.json --mode synapse-simulator",
    ...
    def register(self, app: typer.Typer) -> None:
        ...
        def launch_profile(
            ...
            mode: str = typer.Option("discovery", "--mode", "-m", help="Modo de página: 'discovery' (onboarding), 'landing' (dashboard) o 'synapse-simulator' (dev/test)"),
            ...
            Modo normal: requiere --spec y --mode (discovery, landing o synapse-simulator).
            ...
            # Validación de mode
            valid_modes = ['discovery', 'landing', 'synapse-simulator']
            if mode not in valid_modes:
                ...
                error_msg = (
                    ...
                    "  • synapse-simulator - Entorno de pruebas y desarrollo (dev_mode)\n\n"
                    "Ejemplo:\n"
            ...
            try:
                ...
                if gc.verbose:
                    mode_desc = "ONBOARDING" if mode == "discovery" else ("DASHBOARD" if mode == "landing" else "SYNAPSE_SIMULATOR")
