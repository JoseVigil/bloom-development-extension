...
// priorityPosture builds a ResolvedPosture whose GravityPosture.Expression is
...
func priorityPosture(t *testing.T, postureID, nodeID, expression string) ResolvedPosture {
	t.Helper()
...
func TestDetectPriorityCyclesConfirmsSimpleTwoCycle(t *testing.T) {
	...
	if !reflect.DeepEqual(f.PostureIDs, []string{"r1", "r2"}) {
...
func TestDetectPriorityCyclesCatchesInternalCycleWithinOneExpression(t *testing.T) {
	...
	assertClosedCycle(t, findings[0].Cycle)
	if !reflect.DeepEqual(findings[0].PostureIDs, []string{"r1"}) {
		t.Fatalf("unexpected postureIds: %+v", findings[0].PostureIDs)
	}
}
...
func TestDetectPriorityCyclesNonPriorityAndUnparseablePosturesAreIgnored(t *testing.T) {
	...
	postures := []ResolvedPosture{
		...
		{GravityPosture: GravityPosture{PostureID: "r1", Primitive: "threshold", Expression: thresholdExpr, Status: "active"}, NodeID: "n1"},
		// Missing expression entirely.
		{GravityPosture: GravityPosture{PostureID: "r2", Primitive: "priority", Status: "active"}, NodeID: "n2"},
