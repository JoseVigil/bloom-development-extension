...
// materialización no re-verifica la firma del delivery (§1 del encargo), así que los tests
// no necesitan una firma Ed25519 válida, sólo el JSON que Accept ya habría escrito.
...
// ── loadLatestMandateReceipt ──────────────────────────────────────────────

// TestLoadLatestMandateReceiptMissing cubre §4: "recibo inexistente para el mandateId
// dado → error claro, sin tocar el filesystem de mandates".
func TestLoadLatestMandateReceiptMissing(t *testing.T) {
	appDataDir := t.TempDir()
	_, err := loadLatestMandateReceipt(appDataDir, installTestOrgID, installTestInstallationID, "does-not-exist")
	if err == nil {
		t.Fatal("expected error for missing receipt")
	}
}

// TestLoadLatestMandateReceiptPicksMostRecentVersion cubre §4: "con más de un version-*
// bajo el mismo mandate-{hash} → toma el de AcceptedAt más reciente".
func TestLoadLatestMandateReceiptPicksMostRecentVersion(t *testing.T) {
	appDataDir := t.TempDir()
	mandateID := "m-multi"
	...
	receipt, err := loadLatestMandateReceipt(appDataDir, installTestOrgID, installTestInstallationID, mandateID)
	if err != nil {
...
// ── extractMandateContent ─────────────────────────────────────────────────

func TestExtractMandateContentValid(t *testing.T) {
	...
	gotDelivery, gotContent, err := extractMandateContent(receipt)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
// TestExtractMandateContentCorruptDigest cubre §4: "recibo con body_base64 corrupto
// (digest no coincide) → error duro, nada se escribe".
func TestExtractMandateContentCorruptDigest(t *testing.T) {
	content := []byte(`{"status":"signed","mandateId":"m1"}`)
	delivery := mandatedelivery.Delivery{
	...
	if _, _, err := extractMandateContent(receipt); err == nil {
		t.Fatal("expected error for digest mismatch")
	}
}

func TestExtractMandateContentInvalidBase64(t *testing.T) {
	delivery := mandatedelivery.Delivery{
...
// ── materializeMandateFile ────────────────────────────────────────────────

// TestMaterializeMandateFileNewWrite cubre §4: "recibo válido, mandate.json no existe
// todavía → se crea con el contenido exacto (comparar bytes contra un fixture)".
func TestMaterializeMandateFileNewWrite(t *testing.T) {
	root := t.TempDir()
	content := []byte(`{"status":"signed","mandateId":"m1"}`)

	path, already, err := materializeMandateFile(root, "m1", content)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if already {
...
// TestMaterializeMandateFileIdempotent cubre §4: "re-ejecutar el comando con el mismo
// recibo → idempotente, no reescribe, reporta éxito".
func TestMaterializeMandateFileIdempotent(t *testing.T) {
	root := t.TempDir()
	content := []byte(`{"status":"signed","mandateId":"m1"}`)

	path1, already1, err := materializeMandateFile(root, "m1", content)
	if err != nil {
	...
	path2, already2, err := materializeMandateFile(root, "m1", content)
	if err != nil {
...
// TestMaterializeMandateFileContentConflict cubre §4: "mandate.json ya existe con
// contenido distinto al del recibo → error duro, no se pisa".
func TestMaterializeMandateFileContentConflict(t *testing.T) {
	root := t.TempDir()
	original := []byte(`{"status":"signed","mandateId":"m1","v":1}`)
	conflicting := []byte(`{"status":"signed","mandateId":"m1","v":2}`)

	if _, _, err := materializeMandateFile(root, "m1", original); err != nil {
		t.Fatalf("unexpected error on first write: %v", err)
	}
	if _, _, err := materializeMandateFile(root, "m1", conflicting); err == nil {
