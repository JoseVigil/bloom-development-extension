...
func NewRoot(c *core.Core) *cobra.Command {
	var jsonHelp bool
	root := &cobra.Command{Use: "impact", Short: "Portable consequence evaluator", Long: "Impact evaluates explicit questions without granting authority or executing actions.", Args: cobra.NoArgs, Example: "  impact --help\n  impact --json-help", Annotations: map[string]string{"category": "SYSTEM", "json_response": `{"name":"impact","commands":[],"flags":[]}`}, SilenceErrors: true, SilenceUsage: true}
	root.SetOut(c.Out)
	root.SetErr(c.Err)
...
func Execute(args []string, out, stderr io.Writer, factory func(bool) (*core.Logger, error)) (err error) {
	c := &core.Core{Engine: evaluation.Default(), Out: out, Err: stderr, LoggerFactory: factory}
	root := NewRoot(c)
	root.SetArgs(args)
	...
	if logErr != nil {
		return emitError(c, fmt.Errorf("Impact logging or telemetry initialization failed: %w", logErr))
	}
	c.Logger = logger
	defer func() {
		if closeErr := logger.Close(); closeErr != nil {
			fmt.Fprintln(stderr, "Impact log close or telemetry update failed")
			if err == nil {
				err = closeErr
			}
		}
...
func emitError(c *core.Core, err error) error {
	if c.JSON {
		_ = json.NewEncoder(c.Out).Encode(map[string]string{"error": "evaluation command failed", "detail": err.Error()})
	} else {
