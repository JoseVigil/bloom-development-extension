...
func (l *Logger) rotate(now time.Time) error {
	...
	if l.file == nil || l.day != day {
		dir := filepath.Join(l.options.LogsDir, "impact")
		if err := os.MkdirAll(dir, 0755); err != nil {
			return err
		}
		f, err := os.OpenFile(filepath.Join(dir, "impact_core_"+day+".log"), os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0600)
		if err != nil {
			return err
		}
		if l.file != nil {
...
func (l *Logger) write(level, message string) error {
	...
	if err := l.rotate(now); err != nil {
		fmt.Fprintln(l.options.Errors, "Impact log rotation failed")
		return err
	}
	line := fmt.Sprintf("%s [%s] %s\n", now.Format("2006/01/02 15:04:05"), level, message)
	if _, err := io.WriteString(l.file, line); err != nil {
		fmt.Fprintln(l.options.Errors, "Impact log write failed")
		return err
	}
	if err := l.file.Sync(); err != nil {
		return err
...
func (l *Logger) Close() error {
	...
	if registerErr != nil {
		fmt.Fprintln(l.options.Errors, "Impact telemetry update failed at close")
		return registerErr
	}
	if syncErr != nil {
		return syncErr
