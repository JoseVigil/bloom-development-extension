...
var OwnershipSchema = external_exports.object({
  organization_fingerprint: external_exports.string().regex(/^bloom:org:[a-z0-9-]+$/),
  organization_name: external_exports.string(),
  master_user: external_exports.string(),
  key_fingerprint: external_exports.string(),
...
function buildOrgContext(orgName) {
...
var PathResolver = class {
  ...
  get batcaveData() {
    return path2.join(this.org.batcaveRoot, ".data");
  }
  get batcaveLogs() {
    return path2.join(this.org.batcaveRoot, ".logs");
  }
  get batcaveConfig() {
  ...
  // Governance paths
  get governance() {
    return path2.join(this.org.nucleusRoot, ".governance");
  }
  get securityStandards() {
    return path2.join(this.governance, ".security", ".security-standards.bl");
  }
  // Intent paths
  get intents() {
    return path2.join(this.org.nucleusRoot, ".intents");
  }
  // Reports paths
  get reports() {
    return path2.join(this.org.nucleusRoot, "reports");
  }
  get exports() {
    return path2.join(this.reports, "exports");
  }
  // Dynamic data paths
  tunnelState(tunnelId) {
    return path2.join(this.batcaveData, "tunnels", `${tunnelId}.json`);
  }
  sessionData(sessionId) {
    return path2.join(this.batcaveData, "sessions", `${sessionId}.json`);
  }
  nonceCache(nonceId) {
    return path2.join(this.batcaveData, "nonces", `${nonceId}.json`);
  }
  // Log paths
  governanceLog(date) {
    return path2.join(this.batcaveLogs, "governance", `${date}.log`);
  }
  securityLog(date) {
    return path2.join(this.batcaveLogs, "security", `${date}.log`);
  }
  relayLog(date) {
    return path2.join(this.batcaveLogs, "relay", `${date}.log`);
  }
};
...
var BatcaveConfigSchema = external_exports.object({
  server: external_exports.object({
    port_rest: external_exports.number().default(48215),
    port_wss: external_exports.number().default(4124),
    host: external_exports.string().default("0.0.0.0")
  }).optional(),
  github: external_exports.object({
    oauth_client_id: external_exports.string().optional(),
    oauth_client_secret: external_exports.string().optional(),
    callback_url: external_exports.string().optional()
  }).optional(),
  security: external_exports.object({
    nonce_ttl_seconds: external_exports.number().default(300),
    session_ttl_seconds: external_exports.number().default(86400)
  }).optional(),
  logging: external_exports.object({
    level: external_exports.enum(["debug", "info", "warn", "error"]).default("info"),
    pretty: external_exports.boolean().default(true)
  }).optional()
});
async function loadConfig(org) {
  let fileConfig = {};
  if (existsSync2(org.configPath)) {
    const content = await readFile2(org.configPath, "utf-8");
...
// main.ts
async function main() {
  console.log("\u{1F987} BATCAVE - Sovereign Control Plane");
  console.log("=====================================\n");
  try {
    console.log("\u{1F50D} Resolving organization...");
...
`);
    ...
    console.log("\u{1F4C1} Path resolver initialized");
