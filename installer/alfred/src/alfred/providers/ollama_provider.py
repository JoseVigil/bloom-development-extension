"""Arm de Ollama de Alfred — SOLO genera embeddings.

Mirror deliberado de brain/core/bisp/ollama_manager.py (OllamaManager
real): mismo endpoint (/api/embeddings), mismo modelo (nomic-embed-text),
misma dimensión (768), mismo timeout (30s), mismo patrón de factory
(`from_nucleus_path`) y de auto-arranque opcional.

Dos diferencias deliberadas respecto al original:

1. El contrato se recorta a lo que Alfred necesita acá — nunca
   chat/generate (eso lo cubre gemini_provider.py).
2. Levanta ProviderError con el ErrorCode real del catálogo
   ...
   en vez de RuntimeError con texto libre, para que capas posteriores
   puedan inspeccionar el código sin parsear mensajes.

Migrado desde agentic-harness/harness/providers/ollama_provider.py sin
...
from alfred.providers.base import EmbeddingProviderArm, ProviderHealth
...
# Linux confirmado en este equipo: ~/.local/share/BloomNucleus/bin/ollama/ollama.
