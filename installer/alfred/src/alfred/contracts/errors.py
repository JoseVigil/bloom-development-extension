"""Mirror del repo root: contracts/errors.ts — ERROR_CATALOG completo.

Se transcribe el catálogo entero: distintas partes de Alfred (providers,
router, gate, logging) necesitan severity/retry_strategy de códigos que no
son solo AI_EXECUTION_OLLAMA_* sin tener que volver a este archivo a
ampliarlo pieza por pieza. Es transcripción de datos ya definidos, no
diseño nuevo.

Se omite `docs_url` de cada entry: apunta a rutas /docs/... servidas por la
app de Bloom, no por Alfred — mantenerlo hubiera sido un link muerto fuera
de contexto.

Fuente real: contracts/errors.ts en la raíz del repo. Si ese archivo
cambia, correr los tests de contrato para detectar drift en los campos
cubiertos.

Migrado desde agentic-harness/harness/contracts/errors.py sin cambios de
...
class ProviderError(RuntimeError):
    ...
    Los arms de Alfred (OllamaEmbeddingProvider, GeminiTextProvider) la
    levantan en vez de RuntimeError con texto libre, para que capas
    posteriores (router, gate, logging) puedan inspeccionar
    `error.response.error_code` y decidir retry_strategy programáticamente
    en vez de parsear mensajes.
    """
