...
func TestJSONStdoutSuccessFailureAndHelp(t *testing.T) {
	for i, args := range [][]string{{"status", "--json"}, {"--json-help"}, {"status", "--json-help"}, {"does-not-exist", "--json"}} {
		out, stderr, err := run(t, args...)
		if (i == 0 || i == 1 || i == 2) && err != nil {
...
func TestMetadataIsRecursiveAndNewCommandsAppear(t *testing.T) {
	...
	verify(root)
	parent := &cobra.Command{Use: "future", Annotations: map[string]string{"category": "SYSTEM", "json_response": "{}"}}
	parent.AddCommand(&cobra.Command{Use: "nested", Short: "new command", Annotations: map[string]string{"category": "SYSTEM", "json_response": "{}"}})
...
func TestJSONErrorsBeforeCobraDispatch(t *testing.T) {
	for _, args := range [][]string{{"does-not-exist", "--json"}, {"--bad-flag", "--json"}, {"status", "--json-help", "--bad-flag"}} {
		out, stderr, err := run(t, args...)
