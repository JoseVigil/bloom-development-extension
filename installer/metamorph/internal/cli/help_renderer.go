...
func (r *ModernHelpRenderer) printUsageSection() {
	...
	r.writeln("  " + Bold.Apply("metamorph", r.useColors) + " [OPTIONS] <command> [args]")
	r.writeln("")

	r.writeln("  " + Dim.Apply("Quick examples:", r.useColors))
	r.writeln("    " + Green.Apply("metamorph version", r.useColors) +
		Dim.Apply("                    # Display version information", r.useColors))
	r.writeln("    " + Green.Apply("metamorph info", r.useColors) +
		Dim.Apply("                       # Show system information", r.useColors))
	r.writeln("    " + Green.Apply("metamorph --json info", r.useColors) +
...
func (r *ModernHelpRenderer) printCommandDetail(cmd *cobra.Command) {
	...
	usage := fmt.Sprintf("metamorph %s", cmd.Use)
	...
	if cmd.HasSubCommands() {
		...
		for _, subcmd := range cmd.Commands() {
			...
			subUsage := fmt.Sprintf("metamorph %s %s", cmd.Name(), subcmd.Use)
			r.writeln(fmt.Sprintf("      %s  %s",
...
func (r *ModernHelpRenderer) printSubcommandDetail(parentName string, subcmd *cobra.Command) {
	...
	usage := fmt.Sprintf("metamorph %s %s", parentName, subcmd.Use)
	r.writeln("        " + Dim.Apply("Usage:", r.useColors) + " " + Green.Apply(usage, r.useColors))
	r.writeln("")
...
func (r *ModernHelpRenderer) printFooter() {
	...
	r.writeln(r.centerText(
		Dim.Apply(emoji+" Metamorph: Governance Layer for Bloom Organization", r.useColors),
		r.width))
	r.writeln(r.centerText(
		Dim.Apply("Use 'metamorph <command> --help' for detailed command information", r.useColors),
		r.width))
	r.writeln("")
}
