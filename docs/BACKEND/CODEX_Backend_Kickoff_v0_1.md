...
- `docs/METAMORPH/metamorph-ionpump-reference.md` — describe el subsistema IonPump; **el contrato real confirmado contra el código** (no contra este doc ni contra el README de Metamorph) es `{schema_version, generated_at, ions: []}`, implementado en `installer/metamorph/internal/ionpump/manifest.go`. El manifest que arme el backend para ion recipes tiene que calzar con esto — no con `artifacts[]`, que es una descripción aspiracional del README de Metamorph nunca implementada (`generate-manifest` es un stub que devuelve `not_implemented`, y `reconcile` solo existe dentro de `internal/ionpump/`, no para binarios).
