...
### 1. Eliminación de Panel Lateral
- Remover completamente el panel derecho del formulario
- Layout: 100% para formulario (eliminar grid 70/30)
- Archivos se abren en VSCode, no en preview interno

### 2. Botones de Archivos - 3 Acciones
...
### 4. Gestión Dinámica de Archivos
...
### intent.json (Estructura Minimalista)
...
Métodos principales:
    ...
    class IntentSession {
      ...
      // Ciclo de vida
      async changeStatus(newStatus: 'draft' | 'generated'): Promise<void>
...
Cambios:
- Eliminar div.form-right completamente
- Cambiar container a layout simple (sin grid)
...
Cambios:
- Remover estilos de .form-right
- Remover grid de .container
...
Cambios:
- Eliminar lógica de preview panel
...
- Eliminar método handleGetFileContent
...
### package.json - Comandos a agregar
...
## 🌳 Tree de Archivos Completo
    ...
    │   │
    │   ├── core/
    │   │   ├── metadataManager.ts          ← MODIFICAR (nueva estructura intent.json + método updateContent())
    ...
    │   │
    │   ├── ui/
    │   │   ├── intentForm.html             ← MODIFICAR (eliminar panel derecho)
    │   │   ├── intentForm.css              ← MODIFICAR (layout 100%, botones)
    │   │   ├── intentForm.js               ← MODIFICAR (5 acciones file pill + llamar auto-save)
    │   │   ├── intentFormPanel.ts          ← MODIFICAR (integrar IntentSession)
    │   │   └── [otros existentes...]
    │   │
    │   ├── providers/
    │   │   └── intentTreeProvider.ts       ← MODIFICAR (2 grupos: Draft/Generated)
    │   │
    │   ├── models/
    ...
    └── .bloom/
        ├── scripts/
        │   └── generate_codebase.py        ← OPCIONAL (script Python para regeneración)
        └── intents/
            ├── fix_login_bug/
            │   ├── intent.json
            │   ├── intent.bl
...
- TreeView muestra 2 grupos (Draft/Generated)
...
- Fase 3 (Ciclo de Vida): 1-2 horas
