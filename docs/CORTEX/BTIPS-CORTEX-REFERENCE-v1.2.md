# BTIPS — BLOOM CORTEX REFERENCE v1.2
**Fuente de verdad técnica de la Chrome Extension Bloom Cortex**
*Platform Engineering · Junio 2026*
...
18. [Logging y debugging](#18-logging-y-debugging)
19. [Extensión de capacidades — Guía para nuevos desarrollos](#19-extensión-de-capacidades--guía-para-nuevos-desarrollos)
...
| `ACCOUNT_REGISTERED` | Discovery | Despachado por `REGISTERED_HANDLERS` — ver sección 14 |
...
chrome.runtime.sendMessage({
  ...
  profile_id: string,
  launch_id: string,
  service: 'github' | 'google' | 'gemini' | 'github',
  email: string,
  timestamp: number
})
```
...
Los schemas y los manifests JS deben mantenerse en sincronía. La dirección canónica es `schema → manifest`: cuando se agrega o modifica un campo en el schema, el manifest JS debe actualizarse para reflejarlo. No al revés. Los schemas viven en `extension/protocols/` y son parte del artefacto `.blx`; los manifests JS son su proyección en el contexto `self.*` del Service Worker.
...
El manifest original de `discoveryProtocol.js` definía `ACCOUNT_REGISTERED` sin los campos `service` ni `token_fingerprint`. Sin embargo, `background.js` tenía una guarda en línea ~1171:
...
```json
{
  ...
  "parameters": [
    { "name": "service", "type": "enum", "options": ["github"], "default": "github" },
    { "name": "token_fingerprint", "type": "string", "default": "ghp_simulatedToken" },
    ...
  ]
}
```
...
`loadScriptOptional()` verifica primero la existencia del archivo con un `fetch HEAD` antes de inyectarlo como `<script>`. Si el archivo no existe (caso normal pre-onboarding para los archivos de landing), resuelve sin error. Esta estrategia elimina los errores de red en DevTools para archivos opcionales.
...
`REGISTERED_HANDLERS` es un mapa plano indexado por nombre de evento:

```javascript
const REGISTERED_HANDLERS = {};
// Estructura interna:
// {
//   "ACCOUNT_REGISTERED": { schema: <objeto schema>, handlerFn: <función> },
//   ...
// }
```

El mapa se chequea al inicio del listener `chrome.runtime.onMessage`, antes del if-chain. Si el evento está registrado, se despacha y retorna; si no, cae al if-chain original.
...
El `schema` que se pasa es el objeto de mensaje extraído del JSON schema cargado por `loadProtocolSchemas()`. La forma esperada es `{ parameters: [ { name, default? }, ... ] }`.
...
Puntos clave:
- **No muta el original**: retorna una copia shallow via `Object.assign`.
- **Solo rellena ausentes**: un campo con valor explícito (incluso `false` o `""`) no se sobreescribe.
- **Solo aplica `default`**: parámetros sin `default` en el schema no generan efecto si están ausentes.

### Patch al listener chrome.runtime.onMessage

Al inicio del cuerpo del listener, antes del if-chain existente, se agregó el bloque de dispatch registrado:
...
chrome.runtime.onMessage.addListener((msg, sender, sendResp) => {
  ...
  // ...if-chain original sin tocar...
});
```

**Orden de prioridad:** `REGISTERED_HANDLERS` tiene prioridad sobre el if-chain. Un evento registrado nunca llega al if-chain. El if-chain existente no fue modificado.

**Contrato async preservado:** Si el `handlerFn` retorna `true` (indica que llamará `sendResponse` de forma asíncrona), el listener también retorna `true` hacia Chrome — preservando el canal de respuesta abierto. Si el handler no retorna `true`, el listener retorna `undefined` (canal cerrado), igual que el if-chain existente.

**Campo discriminador:** El dispatch usa `msg.event || msg.command` para identificar el evento. Esto cubre tanto los mensajes tipo `{ event: "ACCOUNT_REGISTERED", ... }` como los tipo `{ command: "ALGO", ... }`.

### Carga de schemas — loadProtocolSchemas()

Los schemas se cargan de forma asíncrona al inicializarse el Service Worker:
...
`loadProtocolSchemas()` se llama durante la inicialización del Service Worker. Los handlers se registran en `registerOnboardingHandlers()` inmediatamente después de que el schema esté disponible en memoria.

### Handler actual — ACCOUNT_REGISTERED

El único handler registrado en Fase 2 es `ACCOUNT_REGISTERED`. Se registra así:
...
protocols/discovery.schema.json
  └─ loadProtocolSchemas()                    ← fetch al inicializar el SW
       └─ discoverySchema en memoria
            └─ registerOnboardingHandlers()
                 └─ registerHandler('ACCOUNT_REGISTERED', schema, fn)
                      └─ REGISTERED_HANDLERS['ACCOUNT_REGISTERED'] = { schema, fn }

  [En runtime, mensaje entrante]
  chrome.runtime.onMessage llega msg = { event: 'ACCOUNT_REGISTERED', ... }
  └─ _registeredEvent = 'ACCOUNT_REGISTERED'
       └─ REGISTERED_HANDLERS tiene esa clave
            └─ applySchemaDefaults(msg, schema)
                 └─ patchedMsg: msg con service='github', token_fingerprint='ghp_simulatedToken'
                      └─ handlerFn(patchedMsg, sender, sendResp)
                           └─ guarda msg.service === 'github' → true ✓
...
|------|--------|-------------|
| Fase 1 | ✅ Completada (v1.0) | If-chain existente — todos los handlers en el listener original |
| Fase 2 | ✅ Completada (v1.2) | `REGISTERED_HANDLERS` + `registerHandler` + patch al listener. `ACCOUNT_REGISTERED` migrado |
| Fase 3 | 🔲 Pendiente | Migración de la guarda residual de `ACCOUNT_REGISTERED` del if-chain hacia `registerHandler` completo |

**Nota sobre Fase 3:** La guarda problemática en línea ~1171 del if-chain original (`if (msg.service === 'github' && msg.token_fingerprint)`) sigue en el if-chain. En Fase 2 ya no puede ejecutarse para `ACCOUNT_REGISTERED` porque el dispatch registrado lo intercepta primero. La Fase 3 limpiará ese código residual y formalizará la migración como patrón para otros eventos.
...
> ❌ **No duplicar handlers.** Un evento debe estar en `REGISTERED_HANDLERS` **o** en el if-chain, nunca en ambos. El dispatch registrado tiene prioridad; el if-chain nunca ve un evento ya registrado.

---

## 15. Manifest de la extensión
...
## 16. Generadores de assets estáticos
...
## 19. Extensión de capacidades — Guía para nuevos desarrollos
...
2. Crear el manifest JS `self.NUEVO_PROTOCOL_MANIFEST` en el archivo correspondiente, en sync con el schema.
...
3. **Declarar los campos faltantes en el schema con sus defaults.** Agregar la entrada en `parameters` con `"default": <valor de simulación>`.
4. **Cargar el schema** en `loadProtocolSchemas()` si no está ya siendo cargado.
5. **Llamar a `registerHandler`** con el schema y extraer la lógica del if-chain al handler. El handler recibe el mensaje ya con defaults aplicados.
6. **Verificar que el if-chain no tiene lógica residual para ese evento.** Buscar y remover cualquier rama `if (msg.event === 'MI_EVENTO')` que haya quedado en el if-chain.

> ⚠️ Un evento no puede estar en `REGISTERED_HANDLERS` y en el if-chain simultáneamente. El dispatch registrado tiene prioridad; si ambos coexisten, el if-chain nunca se ejecuta para ese evento — pero el código residual genera confusión. Limpiar siempre.
...
| `ACCOUNT_REGISTERED` | discovery.js → REGISTERED_HANDLERS → host → Brain | Nucleus | ✅ Crítico |
| `HANDSHAKE_CONFIRMED` | background.js → runtime broadcast | Discovery/Landing | No |

---

*BTIPS · BLOOM CORTEX REFERENCE v1.2 · Platform Engineering · Junio 2026*
