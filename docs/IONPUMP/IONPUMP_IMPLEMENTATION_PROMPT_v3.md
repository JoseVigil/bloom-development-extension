...
> (2) Gate explícito para Phase 3 con criterio de desbloqueado. (3) Advertencia sobre dependencia
> Test 3 del harness ↔ Phase 5. (4) Referencias a SYNAPSE_PROTOCOL_MASTER y BTIPS-CORTEX-REFERENCE
> actualizadas. Sin cambios a la arquitectura ni a los contratos de implementación.
...
### **Documentos actualizados (v3.0):**
4. `SYNAPSE_PROTOCOL_MASTER.md` - Fuente de verdad del protocolo y harness. Tests 1–5.
5. `BTIPS-CORTEX-REFERENCE-v1_0.md` - Referencia completa de Cortex: background.js, content.js, Harness.
...
> **Leyenda de estado:**  
> `[ ]` — No iniciado · `[~]` — En progreso · `[x]` — Completado y verificado · `[!]` — Bloqueado  
> El estado de cada phase debe actualizarse manualmente al avanzar en la implementación.  
> Última actualización de estado: **v3.0 — Abril 2026** (auditoría pre-harness; todas las phases sin verificar en ejecución real).
...
**Estado:** `[ ]` No iniciado — **Crítico para harness Test 2** (port file + `harness.synapse.config.js`)

> ⚠️ **Dependencia de harness:** el Test 2 del SYNAPSE_PROTOCOL_MASTER (`sentinel launch` → port file)
> requiere que `SynapseIPCServer` esté implementado y que `SynapseManager` reciba `launch_id`.
> Sin Phase 2, el harness no genera config y el Test 4 (carga de manifests) no puede ejecutarse.
...
> **Gate de desbloqueado:** antes de iniciar esta phase, el implementador debe:
> 1. Explorar `brain/core/intent/` y confirmar el archivo que actúa como dispatcher de intents.
> 2. Documentar en este archivo el nombre real del dispatcher (reemplazar `intent_executor.py` si difiere).
> 3. Solo entonces proceder con la integración.
>
> **Razón:** `brain/core/intent/intent_executor.py` está referenciado en la spec pero **no está
> confirmado en el árbol real del codebase**. Crear el archivo desde cero sin entender el pipeline
> existente rompería la integración de intents.
>
> **Esta phase no bloquea Phases 1, 2, 4, ni 5.**

- [ ] **[Gate]** Explorar `brain/core/intent/` — identificar y documentar el dispatcher real
- [ ] Integrar `IonPumpManager.execute_flow()` como handler de `intent_subtype == "web_automation"`
...
**Estado:** `[ ]` No iniciado — **Crítico para harness Test 3** (`brain ionpump inspect`)

> ⚠️ **Dependencia de harness:** el Test 3 del SYNAPSE_PROTOCOL_MASTER ejecuta `brain ionpump inspect`.
> Ese comando vive en esta phase. Si Phase 5 no está implementada, el Test 3 se omite en la
> secuencia de verificación del harness y se retoma al completar esta phase.
...
**Estado:** `[ ]` No iniciado — depende de Phase 1 y Phase 2 completadas
...
**Estado:** `[ ]` No iniciado — depende de Phases 1–5 completadas
...
*Document version: 3.0 — Auditoría pre-harness · Abril 2026*  
*Supersedes: IONPUMP_IMPLEMENTATION_PROMPT_Complete_Specification v2.0*
