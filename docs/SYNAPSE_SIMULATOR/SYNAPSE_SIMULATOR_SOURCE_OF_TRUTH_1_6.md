# Bloom — SynapseSimulator + IonPump: Fuente de Verdad
## Versión consolidada · v1.5 — Jul 12 2026 (VAULT_INITIALIZED — institucionalización, 6ta instancia del patrón transversal §5)
### Supersede: todos los documentos del directorio `/docs/SYNAPSE_SIMULATOR/`
...
> `milestone-registry.js`). No requiere cambios en `synapse_simulator_schema.json` (protocolo `ionpump`) ni en
> `synapse_simulator_generator.py`. Hallazgo principal: `discoveryProtocol.js` (legacy manifest ya identificado
...
> en cualquier contexto donde `synapse-simulator.js` priorice el legacy global sobre el JSON. Ver §25 para el
> detalle completo.
...
> `discovery.schema.json` (la fuente de verdad real que lee el SynapseSimulator vía `fetch`) ni a este
> documento. Resultado: el SynapseSimulator seguía ofreciendo simular un flujo que ya no existe, y no tenía
...
> evento sintético del SynapseSimulator que solo se dispara si se simula manualmente. Ver §22 para el
...
> evento sintético que describe ya no está disponible para simular desde el SynapseSimulator.

> **Jerarquía de fuentes para esta revisión:**
> 1. `background.js` — verificado Jun 25 2026 (1680 líneas)
> 2. `synapse-simulator.js` — verificado Jun 25 2026 (746 líneas)
...
> 7. `SYNAPSE_SIMULATOR_SOURCE_OF_TRUTH.md` — revisión anterior Jun 19 2026 (secciones no re-verificadas)
>
> **No verificados en esta ronda** (no provistos): `synapse-simulator/index.html`, `manifest.json`, `synapseSimulatorProtocol.js`, `bloom-host`, archivos de Brain, `seed.go`, `ignition_identity.go`. Lo que este documento afirma sobre esos archivos proviene de la revisión Jun 19 o de revisiones anteriores.

---

## Índice

1. [Qué es el SynapseSimulator y qué es IonPump](#1-qué-es-el-synapse-simulator-y-qué-es-ionpump)
...
4. [Arquitectura del SynapseSimulator](#4-arquitectura-del-synapse-simulator)
...
## 1. Qué es el SynapseSimulator y qué es IonPump
...
### Las tres páginas que el SynapseSimulator puede observar/simular
...
| **SynapseSimulator** (`synapse-simulator/index.html`) | Solo en builds `--dev` | No tiene manifest propio | Lee los tres de arriba en runtime |

`synapse-simulator.js` intenta cargar los tres protocolos en el boot usando `loadScriptOptional()`, que resuelve sin error si el archivo no existe todavía.

### Relación entre SynapseSimulator e IonPump

Son problemas ortogonales que comparten infraestructura. La superficie compartida es que ambos necesitan que Cortex exponga manifests de protocolo legibles en runtime. IonPump define el `SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST`. El SynapseSimulator lo lee. Relación unidireccional: IonPump produce, SynapseSimulator consume.
...
**El SynapseSimulator no tiene protocolo propio.** Lee los protocolos existentes en runtime. Es un lector, no un duplicador.

**El manifest es el contrato.** Cada protocolo expone un `*_PROTOCOL_MANIFEST` autodescriptivo. El SynapseSimulator genera UI desde ese manifest. Agregar features al protocolo actualiza el SynapseSimulator automáticamente.

*(actualizado Jun 26 2026) — A partir de la Fase 1, los manifests viven en JSON schemas independientes en `extension/protocols/` en lugar de globals `self.*_PROTOCOL_MANIFEST` en archivos `*Protocol.js`. El SynapseSimulator los carga via `ProtocolReader.discoverFromJSON()`. El principio es el mismo; el mecanismo de entrega cambió.*

**Los canales son tipos, no hardcoding.** El manifest diferencia mensajes de `runtime` y de `tabs`. El SynapseSimulator selecciona el mecanismo de dispatch correcto según el tipo. *(Nota Jun 25 2026: en `synapseSimulatorProtocol.js` el canal `"tabs"` todavía no aparece — los 10 mensajes existentes son todos `"runtime"`. Ver §6.5 y §18 pregunta #7.)*

**Dev/prod por construcción, no por flags.** El SynapseSimulator existe en builds dev porque Brain lo genera en seed. No existe en prod porque Brain no lo genera.

**Re-seed como mecanismo de actualización.** Cambios en el SynapseSimulator se aplican con un re-seed.
...
| Componente | Rol en SynapseSimulator/IonPump |
|---|---|
| **Brain** | Aloja `IonPumpManager` (runtime). Expone admin CLI. Genera `synapse_simulator_generator.py` en seed. Es el único escritor del `extensionDir`. |
| **Sentinel** | Pasa flag `--dev` a `brain profile create` en seed. Escribe `synapse-simulator.synapse.config.js` en launch (no en seed). No toca `extensionDir` después de llamar a Brain. |
...
El SynapseSimulator vive en Brain templates. Brain lo copia durante el seed.
...
└── synapse_simulator_generator.py      ← genera/copia el template en seed
...
En prod, `generate_synapse_simulator_page()` es un no-op.

### 4.2 Los 3 paneles del SynapseSimulator
...
**Panel central — Simulate:** campos editables para parámetros `string`/`enum`, preview JSON en tiempo real, botón **Send** que despacha via `chrome.runtime.sendMessage`. Parámetros `type: "auto"` se resuelven desde `SYNAPSE_SIMULATOR_CONFIG`/`SYNAPSE_CONFIG` — no son editables.

**Panel derecho — Log / Config:** stream de `INFO`, `SEND`, `ACK`, `ERR`. Tab Config: muestra estado de `SYNAPSE_SIMULATOR_CONFIG` y `SYNAPSE_CONFIG`.

### 4.3 Cómo abrir el SynapseSimulator

```
chrome-extension://<ID>/synapse-simulator/index.html
```
...
4. Al menos un launch fue ejecutado (para que `synapse-simulator.synapse.config.js` exista)

Dev Tools del SynapseSimulator: `chrome://extensions` → Bloom Nucleus Bridge → **Inspect views** → `synapse-simulator/index.html`

### 4.4 ProtocolReader — implementación real (verificado Jun 25)

`ProtocolReader` es un objeto literal (no una clase) en `synapse-simulator.js`:
...
`Simulator.send()` en `synapse-simulator.js` despacha via `chrome.runtime.sendMessage(payload)`. El canal `"tabs"` no está implementado en ningún manifest activo (todos los mensajes actuales son `"runtime"`).

### 4.6 SYNAPSE_SIMULATOR_HELLO / SYNAPSE_SIMULATOR_REPLAY — mecanismo de buffer (verificado Jun 25)

`background.js` implementa un buffer circular de 100 entradas (`synapseSimulatorLogBuffer`). Cuando el SynapseSimulator abre y manda `SYNAPSE_SIMULATOR_HELLO`, `background.js` responde con todo lo acumulado:
...
Esto resuelve la condición de carrera: `HANDSHAKE_CONFIRMED`, `EXTENSION_LOADED` y otros eventos críticos se emiten antes de que `openSynapseSimulatorTab()` haya terminado de cargar. Sin buffer, el SynapseSimulator los perdería.
...
1. **POST** `http://localhost:48215/api/internal/system-event` → nucleus Control Plane → Workspace SynapseSimulator
2. **`chrome.runtime.sendMessage(synapseSimulatorMsg)`** → Cortex SynapseSimulator tab

Ambos son fire-and-forget. El mensaje también se guarda en `synapseSimulatorLogBuffer` independientemente de si alguien está escuchando.

Tokens y keys son sanitizados antes de enviarse (primeros 10 caracteres + `…`). Keepalive y HEARTBEAT son excluidos del log.

### 4.8 Apertura automática del SynapseSimulator tab (verificado Jun 25)

```javascript
// En background.js — handler de host_ready:
if (config?.synapseSimulator) {
  openSynapseSimulatorTab();   // solo si synapse-simulator/index.html existe (config.synapseSimulator != null)
}
...
La apertura del SynapseSimulator ocurre en **FASE 2 del handshake** (`host_ready` recibido), no en el boot del SW. Esto garantiza que el canal Native Messaging ya está establecido cuando el SynapseSimulator abre.
...
> *(actualizado Jun 26 2026)* — A partir de la Fase 1–5, los manifests de protocolo viven en `extension/protocols/*.schema.json` y son la única fuente autorizada de estructura, defaults y metadata de mensajes. Los globals `self.*_PROTOCOL_MANIFEST` documentados en §6.1–§6.5 siguen siendo válidos como descripción del contrato de mensajes, pero su mecanismo de entrega cambió: ya no viven en archivos `*Protocol.js` sino en JSON schemas cargados via `chrome.runtime.getURL()` + `fetch()`. Ver ARCHITECTURE_SynapseSimulatorProtocol.md para la especificación completa del formato JSON schema y el wrapper `registerHandler` en background.js.

### 6.1 Principio

La versión anterior del SynapseSimulator definía una tabla estática de mensajes que era una copia manual del contrato en `discoveryProtocol.js`. Eso es duplicación. El principio correcto: el SynapseSimulator no tiene protocolo propio — lee los protocolos existentes en runtime.
...
| Tipo | Comportamiento |
|---|---|
| `type: "auto"` + `source` | Se resuelve desde config activo (`SYNAPSE_SIMULATOR_CONFIG`, `SYNAPSE_CONFIG`). Invisible al developer en el SynapseSimulator. |
...
self.LANDING_PROTOCOL_MANIFEST = {
  ...
  messages: [
    {
      id: "profile_load", type: "command", direction: "synapse_simulator_to_background", channel: "runtime",
      description: "Request a full profile data reload",
      payload_template: { command: "profile_load", profile_id: "$PROFILE_ID" },
      parameters: [{ name: "profile_id", type: "auto", variable: "$PROFILE_ID", source: "SYNAPSE_SIMULATOR_CONFIG.profileId" }]
    },
    {
      id: "health_check", type: "command", direction: "synapse_simulator_to_background", channel: "runtime",
    ...
    {
      id: "nucleus_sync", type: "command", direction: "synapse_simulator_to_background", channel: "runtime",
    ...
    {
      id: "intent_list", type: "command", direction: "synapse_simulator_to_background", channel: "runtime",
    ...
    {
      id: "session_status", type: "event", direction: "synapse_simulator_to_background", channel: "runtime",
    ...
    {
      id: "stats_update", type: "event", direction: "synapse_simulator_to_background", channel: "runtime",
...
Landing solo aparece en el SynapseSimulator cuando `landing/landingProtocol.js` existe en el `extensionDir` — es decir, post-onboarding. `synapse-simulator.js` lo intenta cargar al final de su secuencia de boot (§10.5) y no falla si no está.

### 6.5 SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST — estructura real (verificado contra fuente, Jun 19 2026)

> **Corrección respecto a versiones anteriores de este documento:** la estructura documentada acá antes (un array `messages` con `site_ready`/`response_ready` por `chrome.runtime.sendMessage`, más un array separado `tab_messages` con `dom_focus`/`dom_type` por `chrome.tabs.sendMessage`) **no existe en el código real**. El archivo fuente (`synapseSimulatorProtocol.js`) usa un único array `messages` con **10 comandos**, todos `channel: "runtime"` — no hay segundo array `tab_messages` ni canal `"tabs"` implementado. Si la intención original (background → content vía `chrome.tabs.sendMessage`) sigue siendo válida, todavía no está en el manifest; lo que el SynapseSimulator puede simular hoy son comandos DOM dirigidos por `tab_id` como parámetro de texto, no por canal de tabs real.
...
`tab_id` aparece como parámetro `type: "string"` con default `"1"` en los comandos DOM — es texto libre que el operador del SynapseSimulator completa a mano, no se resuelve automáticamente contra la tab activa. `launch_id` sí es `type: "auto"`, resuelto desde `SYNAPSE_CONFIG.launchId`.
...
- **SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST:** agregar `perplexity.ai` al campo `options` de los parámetros `site`.
- **El SynapseSimulator no se toca.** ProtocolReader refleja el cambio automáticamente en runtime.
...
sentinel seed <alias> <master> --dev
  ...
  │           └── discovery_generator.py: copia discoveryProtocol.js, synapseSimulatorProtocol.js
  │           └── synapse_simulator_generator.py (--dev): copia synapse-simulator/index.html
...
sentinel launch <alias>
  ...
  └── ignition_identity.go::prepareSessionFiles()
        ...
        └── writeSynapseSimulatorConfig()     ← NUEVO (solo si synapse-simulator/index.html existe)
              escribe synapse-simulator.synapse.config.js con profileId y launchId
```

`synapse-simulator.synapse.config.js` se escribe en **launch**, no en seed. En launch ya existe el `launch_id`. En seed aún no.
...
└── synapse-simulator.synapse.config.js         ← NUEVO (Sentinel, en launch, solo si synapse-simulator existe)
```
...
Esto re-ejecuta `brain profile create --dev` que sobrescribe `synapse-simulator/index.html`. No requiere reinstalar Cortex ni empaquetar un nuevo `.blx`.
...
### 8.4 synapse_simulator_generator.py
...
def generate_synapse_simulator_page(target_ext_dir: Path, profile_data: Dict[str, Any], dev_mode: bool = False) -> None:
    ...
    synapse-simulator.synapse.config.js es responsabilidad de Sentinel en launch (ignition_identity.go).
    """
    if not dev_mode:
        return
...
### 8.6 synapseSimulatorProtocol.js — ubicación correcta

El archivo vive en `brain/core/profile/web/templates/discovery/synapseSimulatorProtocol.js` y es copiado por `discovery_generator.py` junto con los demás assets estáticos. Sentinel no interviene.
...
`background.js` tiene 1680 líneas. Las secciones relevantes para SynapseSimulator/IonPump:

- **Líneas 33–89:** `DEBUG_API_URL`, `synapseSimulatorLogBuffer`, `pushSynapseSimulatorLog()`, `forwardToDebugPanel()` — doble destino
- **Líneas 357–411:** `loadSynapseSimulatorConfig()` — carga `synapse-simulator.synapse.config.js`, activo siempre (no requiere `--dev`)
- **Líneas 413–431:** `openSynapseSimulatorTab()` — busca tab existente o crea nueva
- **Líneas 433–454:** `openDiscoveryTab()` — abre/recarga discovery tab en `host_ready`
- **Líneas 617–685:** `handleHostMessage()` → rama `host_ready` → handshake FASE 2 y 3, apertura SynapseSimulator y Discovery tabs, escritura de `synapseStatus` en storage
- **Líneas 969–979:** handler `SYNAPSE_SIMULATOR_HELLO` → `SYNAPSE_SIMULATOR_REPLAY`
...
- **Líneas 1216–1234:** handler `GITHUB_TOKEN_STORED` (standalone, para SynapseSimulator y otros callers)
...
FASE 2: Host → Extension
  ...
  → openSynapseSimulatorTab() si config.synapseSimulator existe
...
1. `forwardToDebugPanel('synapse', 'ACCOUNT_REGISTERED', ...)` — log al SynapseSimulator
...
```
Los schemas JSON deben estar en `web_accessible_resources` para que `fetch(chrome.runtime.getURL(...))` funcione tanto desde `background.js` como desde `synapse-simulator.js`. El archivo `discovery/synapseSimulatorProtocol.js` fue removido de esta lista al migrarse al schema JSON.
...
### 10.2 ignition_identity.go — writeSynapseSimulatorConfig en launch

```go
func writeSynapseSimulatorConfig(profileID, launchID, profileAlias, extensionDir string) error {
    // No-op si synapse-simulator/index.html no existe (garantiza que solo corre en perfiles --dev)
...
- NO implementa `copySynapseSimulatorPage()` en seed — eso lo hace Brain/`synapse_simulator_generator.py`
...
3. Abrir el SynapseSimulator: `chrome-extension://<ID>/synapse-simulator/index.html`
...
```
SynapseSimulator (Cortex) — dispatches en orden:
...
3. [simular click "Confirmar" — no se puede desde SynapseSimulator; necesita acción en Discovery]
...
4. account_registered      → profile_id: auto, launch_id: auto
   ...
   separado del SynapseSimulator; despacharlo requiere un paso 4b manual: `github_token_stored`.
...
Todo lo que `forwardToDebugPanel()` emite llega al feed del Workspace SynapseSimulator (`:4124`) Y al feed del Cortex SynapseSimulator (tab de la extensión). En el Cortex SynapseSimulator, el panel Log muestra:
- `[SEND]` — el SynapseSimulator despachó un mensaje
- `[ACK]` — respuesta de `background.js`
- `[INFO]` — logs de ciclo de vida del SynapseSimulator
- `[ERR]` — error de dispatch

Adicionalmente, gracias al `SYNAPSE_SIMULATOR_HELLO`/`SYNAPSE_SIMULATOR_REPLAY`, el SynapseSimulator también recibe retroactivamente todos los eventos que ocurrieron desde el boot del SW hasta que la tab del SynapseSimulator abrió.

### 12.4 Gap conocido — botón "Confirmar" en GithubAuthFlow

El SynapseSimulator no puede simular clicks en la UI de Discovery directamente. El paso "Confirmar token" requiere o:
- Hacer click manual en la página de Discovery, **o**
- Despachar `account_registered` directamente desde el SynapseSimulator (saltea la pantalla de confirmación pero produce el mismo efecto en `background.js`)
...
| `background.js` — `synapseSimulatorLogBuffer` + `SYNAPSE_SIMULATOR_HELLO`/`REPLAY` | ✅ Implementado | Jun 25 |
...
| `synapse-simulator.js` — `ProtocolReader`, `Simulator`, `Logger`, `ConfigReader` | ✅ Implementado | Jun 25 |
| `synapseSimulatorProtocol.js` — `SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST` 10 comandos | ✅ (Jun 19) | Jun 19 |
| `seed.go` — flag `--dev` | ✅ (Jun 19) | Jun 19 |
| `ignition_identity.go` — `writeSynapseSimulatorConfig()` | ✅ (Jun 19) | Jun 19 |
| `nucleus/internal/supervisor/onboarding_synapse_simulator.go` | ❓ No auditado | — |
...
├── synapse-simulator.synapse.config.js       ← Sentinel, en launch (solo si synapse-simulator existe)
├── background.js
...
│   └── synapse-simulator.schema.json         ← fuente de verdad del protocolo ionpump
├── discovery/
│   ├── index.html
│   ├── discovery.js
│   ├── discoveryProtocol.js        ← LEGACY — marcado para eliminación (Fase 5)
│   └── synapseSimulatorProtocol.js         ← LEGACY — marcado para eliminación (Fase 5)
├── landing/
│   ├── index.html
│   ├── landing.js
...
└── synapse-simulator/
    ...
    └── synapse-simulator.js                  ← ProtocolReader, Simulator, Logger, ConfigReader
```
...
- [x] `forwardToDebugPanel()` con doble destino (nucleus + SynapseSimulator tab)
- [x] `synapseSimulatorLogBuffer` — buffer circular 100 entradas
- [x] Handler `SYNAPSE_SIMULATOR_HELLO` → `SYNAPSE_SIMULATOR_REPLAY`
- [x] `openSynapseSimulatorTab()` llamado en `host_ready` cuando `config.synapseSimulator` existe
...
- [x] `ProtocolReader.discoverFromJSON()` en synapse-simulator.js *(Fase 1–5)*
...
- [x] Listener `onboarding_navigate` para navegación remota desde Nucleus/SynapseSimulator
...
- [x] `writeSynapseSimulatorConfig()` en launch
- [ ] Auditar `nucleus/internal/supervisor/onboarding_synapse_simulator.go` (ver §18 #6)
...
4. El SynapseSimulator NO existe en prod.
...
17. `synapse-simulator.synapse.config.js` se escribe en launch, no en seed.
...
9. **`ACCOUNT_REGISTERED` y `GITHUB_TOKEN_STORED` NO tienen relación de cascada.** *(Corregido en v1.2 — ver §22.)* `discovery.js` solo emite `ACCOUNT_REGISTERED` (emisor único: `GithubAuthFlow._saveToken()`). `GITHUB_TOKEN_STORED` no es un evento de producción: es un evento sintético del SynapseSimulator (`discovery.schema.json`, `github_token_stored`, `"direction": "synapse_simulator_to_background"`) que solo se dispara si se simula manualmente desde el panel de testing. El handler de `background.js` para `ACCOUNT_REGISTERED` no contiene ningún `if (service === 'github')` ni emite `GITHUB_TOKEN_STORED`; son dos `if` independientes dentro del mismo listener `chrome.runtime.onMessage`, cada uno con su propio `sendToHost()`. Si se observan ambos eventos casi simultáneos, la causa es que el evento simulado del SynapseSimulator se disparó mientras el flujo real de `discovery.js` corría en paralelo — un artefacto de tener dos fuentes activas a la vez, no una cascada de código.
...
| 6 | ¿Qué es `nucleus/internal/supervisor/onboarding_synapse_simulator.go`? | Abierta — no auditado | No bloqueante |
...
*Revisión Jun 25 2026: auditoría completa contra background.js, synapse-simulator.js, discoveryProtocol.js, discovery.js, landingProtocol.js, landing.js*
...
*Documento consolidado — Junio 2026*  
*Fuente primaria: estado operativo de sesión Jun 6 (`SYNAPSE_SIMULATOR_Manual_Onboarding_Debug.md`)*  
*Correcciones arquitectónicas: `IMPL_PROMPT_CORTEX_SENTINEL_SynapseSimulator_v2.md` + `IMPL_PROMPT_BRAIN_IonPump_SynapseSimulator_v2.md` (Abr 25)*  
*Los documentos anteriores en `/docs/SYNAPSE_SIMULATOR/` pueden archivarse.*
...
`discoveryProtocol.js`, `discovery.js` (completo, 1166 líneas), `onboarding.js` (completo, 438 líneas), `synapse-simulator.js` (completo, 731 líneas), `landingProtocol.js`, `landing.js`, `synapseSimulatorProtocol.js`, `discovery/index.html`, `landing/index.html`.

**No se pudo verificar en esta ronda** (no provistos): `synapse-simulator/index.html`, `manifest.json`, `background.js`, `bloom-host`, cualquier archivo de Brain (`synapse_simulator_generator.py`, `discovery_generator.py`, `synapse_manager.py`), `seed.go`, `ignition_identity.go`. Todo lo que este documento afirma sobre esos archivos sigue siendo lo heredado de versiones anteriores — **no re-verificado**, no asumir que está confirmado solo porque aparece en este documento.
...
3. `SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST` real no tiene canal `"tabs"` ni array `tab_messages` — la estructura documentada en versiones previas no existe en el código. Corregido en §6.5, abierto como pregunta #7 en §18.
4. Existe un mecanismo `SYNAPSE_SIMULATOR_HELLO`/`SYNAPSE_SIMULATOR_REPLAY`/`SYNAPSE_SIMULATOR_LOG` en `synapse-simulator.js` que resuelve la pregunta histórica sobre captura de broadcasts — no documentado en ninguna versión anterior. Nuevo §9.3. Revela la existencia de un "Workspace SynapseSimulator" no documentado en ningún archivo — pregunta #6 en §18.
...
6. La nota de limitación en §12.2 sobre mensajes invisibles para el SynapseSimulator por "límite de arquitectura MV3" ya no es precisa dado el mecanismo de §9.3 — corregida.

---

## 20. Adenda — auditoría Jun 25 2026 (esta revisión)

**Archivos leídos:** `background.js` (1680 líneas), `synapse-simulator.js` (746 líneas), `discoveryProtocol.js` (503 líneas), `discovery.js` (1310 líneas), `landingProtocol.js` (594 líneas), `landing.js`.
...
6. **`forwardToDebugPanel()` confirma doble destino.** El Workspace SynapseSimulator y el Cortex SynapseSimulator reciben el mismo evento simultáneamente. El buffer resuelve la condición de carrera para el Cortex SynapseSimulator.
...
**No verificado en esta ronda:** `synapse-simulator/index.html`, `synapseSimulatorProtocol.js`, `manifest.json`, archivos de Brain, `seed.go`, `ignition_identity.go`, `onboarding_synapse_simulator.go`.
---

## 21. Adenda — Fase 5: migración a JSON schemas (Jun 26 2026)

Esta adenda documenta los cambios arquitectónicos implementados en las Fases 1–5 del SynapseSimulator Protocol Single Source of Truth.
...
### Cambios en synapse-simulator.js
...
| `synapseSimulatorProtocol.js` | `protocols/synapse-simulator.schema.json` |

La secuencia de `loadScriptOptional()` en el boot del SynapseSimulator se elimina junto con estos archivos. Ver ARCHITECTURE_SynapseSimulatorProtocol.md §8 para el orden de limpieza recomendado.
...
**Archivos verificados para esta adenda:** contexto de sesión Jun 26 2026 + ARCHITECTURE_SynapseSimulatorProtocol.md v1.0 Post-Fase 5.
...
`synapse-simulator.schema.json`, `landing.schema.json`.
...
No es un evento de producción. Es un **evento sintético del SynapseSimulator**, definido en
`discovery.schema.json` (mensaje `github_token_stored`) con `"direction": "synapse_simulator_to_background"`
— solo existe para que el panel de testing simule manualmente "el usuario confirmó guardado de
token", sin pasar por el flujo real de `discovery.js`. Aparece también en `synapse-simulator.schema.json`
como opción de `EVENT_EMIT` (simulación vía IonPump/SynapseSimulator), y en `landing.schema.json` como
...
host — pero solo se activa si algo lo dispara manualmente (SynapseSimulator), nunca como parte del flujo
...
simulado `github_token_stored` del SynapseSimulator mientras el flujo real de `discovery.js` corría en
...
| §12.2 (walkthrough SynapseSimulator) | Paso 4 corregido — ya no asume disparo automático de `GITHUB_TOKEN_STORED` |
...
*Corrección aplicada sobre v1.1 (Jun 25 2026) según SYNAPSE_SIMULATOR_SOURCE_OF_TRUTH_FIX.md: elimina la
...
a las 503 líneas verificadas en v1.2), `discovery.schema.json`, `synapse-simulator.schema.json`,
`ARCHITECTURE_SynapseSimulatorProtocol.md` (v1.0, Post-Fase 5, actualizado Jun 26 2026).
...
anterior de este documento: `synapse_simulator_simulate_handshake` (fuerza `handshakeState` a `CONFIRMED` sin
native host) y `synapse_simulator_open_landing` (abre la tab de landing directamente desde el SynapseSimulator).

### El gap que causó el problema original

Según **Regla 4** de `ARCHITECTURE_SynapseSimulatorProtocol.md` (§7), los schemas se cargan exclusivamente
vía `chrome.runtime.getURL(...)` + `fetch()` — nunca se importan los archivos `*Protocol.js` como
fuente para el SynapseSimulator una vez completada la Fase 5. Eso significa que, en la práctica, el SynapseSimulator
...
eliminación (§8 de `ARCHITECTURE_SynapseSimulatorProtocol.md`, "Fase 5") y `ProtocolReader.discover()` solo
...
`GITHUB_APP_AUTHORIZED` / `GITHUB_DEVICE_FLOW_ERROR` desde el SynapseSimulator. La única opción para
...
`client_id` es un problema de configuración de producto, no del SynapseSimulator — pero la ausencia de
simulación en el SynapseSimulator es lo que obligaba a pegarse contra el flujo real para poder seguir
probando el resto del onboarding.

### Corrección aplicada

- `discovery.schema.json` → `1.1.0`: se agregaron `github_device_code`, `github_app_authorized`,
  `github_device_flow_error`, `synapse_simulator_simulate_handshake`, `synapse_simulator_open_landing`; se retiraron
...
- `synapse-simulator.schema.json` → `1.1.0`: el enum de `event_emit.event_name` tenía `"google"` y
...
1. **`GITHUB_APP_CLIENT_ID` sin registrar** en `background-github-device-flow.js` — bug de
   producción real, no de SynapseSimulator. No se toca acá; requiere que alguien cargue el client_id de la
...
2. **`landing.schema.json`** no fue releído en esta revisión — si tiene los mismos eventos de
   ...
   mismo criterio.
3. **`synapseSimulatorProtocol.js`** (el legacy `.js` de IonPump) no incluía `"google"`/`"gemini"` en su
   propio manifest — o sea que ese bug se introdujo *solo* en `synapse-simulator.schema.json`, probablemente
...
4. Confirmar si `discoveryProtocol.js` y `synapseSimulatorProtocol.js` ya pueden borrarse del bundle
   (Fase 5, paso 3 de `ARCHITECTURE_SynapseSimulatorProtocol.md` §8) ahora que el JSON quedó al día, o si
...
exactamente lo mismo que el "Simulate" del SynapseSimulator dentro de la extensión, pero mirando el sistema
desde el lado del host en vez del lado del navegador — de ahí que el usuario lo describa como "la
contraparte de SynapseSimulator en Cortex".
...
`ARCHITECTURE_SynapseSimulatorProtocol.md` (§7) — "nunca hardcodear defaults de mensajes de protocolo en un
...
1. **Sin fuente de verdad compartida.** `debug.html` no lee `discovery.schema.json` ni ningún otro
   ...
   copia empaquetada), sería el mismo cambio de arquitectura que ya se hizo en SynapseSimulator vía Fase 5,
...
corregido en `debug.html` (Conductor Workspace, contraparte de SynapseSimulator del lado Electron/host). Ver
§24.*
...
`prompt_update_synapse_simulator_vault_initialized.md` para el detalle completo de esa verificación; esta
sección resume lo que aporta la ronda específicamente sobre el SynapseSimulator.

### Qué cambia (y qué no) en el SynapseSimulator

- **`synapse_simulator_schema.json` (protocolo `ionpump`) — sin cambios.** No corresponde agregar
...
- **`synapse_simulator_generator.py` — sin cambios.** Solo copia estáticos (`index.html`, `synapse-simulator.js`,
  `synapseSimulatorProtocol.js`); no tiene listas de eventos hardcodeadas, así que no hay drift que corregir
  ahí.
- **`synapse-simulator.js` (`ProtocolReader`) — sin cambios de código, pero relevante para el hallazgo de abajo.**
...
`synapse-simulator.js`: el SynapseSimulator mostraría el manifest viejo (sin `VAULT_INITIALIZED`) aunque el JSON ya esté
...
2. **`synapseSimulatorProtocol.js`** (análogo legacy del protocolo SynapseSimulator) — no estuvo disponible en esta
