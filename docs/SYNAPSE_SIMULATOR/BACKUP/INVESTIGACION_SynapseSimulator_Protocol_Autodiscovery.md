# Investigación: SynapseSimulator Protocol Autodiscovery
## SynapseSimulator como lector de protocolos, no duplicador de contratos

---

## 1. El problema raíz: duplicación de contratos

La versión anterior del SynapseSimulator definía una tabla estática de mensajes que podía simular. Esa tabla era una copia manual del contrato que ya existe en `discoveryProtocol.js` y `landingProtocol.js`. Cada vez que Discovery agregara un nuevo paso de onboarding — un nuevo sitio web, un nuevo evento, una nueva transición de estado — habría que actualizar dos lugares: el protocolo real y la tabla del SynapseSimulator.

Eso es duplicación por definición. Y la duplicación en sistemas de desarrollo se paga en el peor momento: cuando estás debuggeando algo urgente y el SynapseSimulator tiene una versión vieja del contrato.

**El principio correcto:** el SynapseSimulator no tiene un protocolo propio. El SynapseSimulator lee los protocolos existentes en runtime y genera su UI a partir de ellos.
...
Estos archivos definen los eventos, comandos, transiciones de estado, y payloads que el sistema reconoce. Son la fuente de verdad. El SynapseSimulator tiene que leerlos — no copiarlos.
...
Para que el SynapseSimulator pueda leer los protocolos en runtime, los protocolos necesitan exponer una estructura autodescriptiva. No cambia lo que hacen — solo agrega un bloque de metadatos que describe lo que ya hacen.
...
**Principio de diseño:** los parámetros con `type: "auto"` y `source` se rellenan automáticamente desde el config activo. El developer no tiene que tipearlos. Los parámetros con `type: "string"` o `type: "enum"` aparecen como campos editables en el SynapseSimulator.

### 3.2 El Protocol Reader

El SynapseSimulator tiene un módulo `ProtocolReader` que carga los manifests al inicializarse:

```javascript
// synapse-simulator/index.html — bloque interno ProtocolReader
...
### 3.3 Cómo el SynapseSimulator genera la UI del Panel Simulate
...
Cuando se agrega un nuevo mensaje al manifest de Discovery, aparece automáticamente en el Panel Simulate sin tocar el SynapseSimulator.
...
**`chrome.runtime.sendMessage`** — broadcast a todos los listeners de la extensión. Cualquier página de la extensión que tenga `onMessage` lo recibe: background.js, Discovery, Landing, y el SynapseSimulator.

**`chrome.tabs.sendMessage(tabId, msg)`** — mensaje dirigido a un content script corriendo en una tab específica. El SynapseSimulator no lo recibe automáticamente porque no está corriendo en esa tab. Solo lo recibe el content script de esa tab.

En el flujo de onboarding, todo pasa por `chrome.runtime.sendMessage` y el SynapseSimulator puede participar normalmente. En IONPump, el flujo es diferente:
...
El SynapseSimulator ve los mensajes del paso 4 y 5 (los que van por `chrome.runtime`), pero no puede interceder en el paso 3 (el `tabs.sendMessage` dirigido al content script).
...
El SynapseSimulator quiere enviar `DOM_FOCUS`, `DOM_TYPE`, `DOM_CLICK` al content script de claude.ai para probar que el content script ejecuta correctamente. Pero esos mensajes van por `tabs.sendMessage` y el SynapseSimulator necesita conocer el `tabId` de la tab donde está abierto claude.ai.

**Dirección B — simular que el content script completó su trabajo sin tener claude.ai abierto:**
El SynapseSimulator quiere emitir `RESPONSE_READY` como si lo hubiera emitido el content script, para probar que `background.js` lo procesa correctamente y lo reenvía a Brain.

### 4.3 Solución: el SynapseSimulator como Tab-Aware Proxy

Para la Dirección A, el SynapseSimulator necesita un paso previo de descubrimiento de tab:

```javascript
// Panel Config del SynapseSimulator — sección IONPump
...
Para la Dirección B, el SynapseSimulator ya puede hacerlo: `chrome.runtime.sendMessage` broadcast sin necesitar tabId.
...
self.SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST = {
  ...
  // Mensajes que van POR chrome.runtime (el SynapseSimulator puede simularlos directamente)
  runtime_messages: [
    {
      id: "site_ready",
      type: "event",
  ...
  // Mensajes que van POR chrome.tabs.sendMessage (el SynapseSimulator necesita tabId)
  tab_messages: [
    {
      id: "dom_focus",
      type: "command",
    ...
    },
    {
      ...
      parameters: [
        ...
        { name: "text", type: "string", variable: "$TEXT", default: "Test prompt from SynapseSimulator" }
      ]
    }
  ]
};
...
El `ProtocolReader` del SynapseSimulator diferencia los dos tipos de mensaje por el campo `channel`:
...
### 5.3 Lo que cambia en el protocolo para que el SynapseSimulator lo lea

En `SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST`, se agrega perplexity al campo `options` de los parámetros `site`:

```javascript
options: ["claude.ai", "chatgpt.com", "grok.com", "aistudio.google.com", "perplexity.ai"]
```

El SynapseSimulator lo refleja automáticamente en runtime sin ningún cambio en su código.

### 5.4 Lo que NO cambia

- El SynapseSimulator (`synapse-simulator/index.html`) no se toca
...
El patrón es: la fuente de verdad vive en el protocolo. El SynapseSimulator la lee. Solo se actualiza la fuente.

---

## 6. SynapseSimulator en dev vs prod — por qué vive en Brain templates y no en el .blx

### 6.1 La decisión

El SynapseSimulator vive en `brain/core/profile/web/templates/synapse-simulator/` y Brain lo copia durante el seed. No está en el artefacto `.blx` de Cortex.

### 6.2 Por qué

**Razón primaria — separación de versiones dev/prod:** el SynapseSimulator es una herramienta de desarrollo. En producción no debe existir. Si viviera en el `.blx`, habría que tener dos builds de Cortex: uno con SynapseSimulator y uno sin. Eso complica el pipeline de builds de Cortex.

Viviendo en Brain templates, la lógica es trivial: en un build de Brain de producción, `generate_synapse_simulator_page()` es un no-op o simplemente no se llama. El directorio `synapse-simulator/` nunca se crea en el filesystem del usuario de producción. La URL `chrome-extension://{id}/synapse-simulator/index.html` devuelve 404 — que es exactamente lo que querés.

**Razón secundaria — flexibilidad de hidratación:** `generate_synapse_simulator_page()` puede recibir parámetros dinámicos del perfil igual que `generate_discovery_page()`. Si el SynapseSimulator en el futuro necesita inyectar datos del perfil en el HTML (feature flags, flags de debug, rutas de logs), Brain puede hacerlo en el momento del seed. Con el `.blx`, eso es imposible sin un paso de postprocesado.

**Razón terciaria — ciclo de actualización independiente:** el SynapseSimulator puede actualizarse sin tocar Cortex. Si agregás una feature al SynapseSimulator, hacés un re-seed. No hay que empaquetar, firmar y distribuir un nuevo `.blx`. El ciclo de desarrollo del SynapseSimulator es local y rápido.

### 6.3 El argumento del re-seed para refrescar

El re-seed del mismo perfil sobreescribe `synapse-simulator.synapse.config.js` y reemplaza `synapse-simulator/index.html`. Si el developer actualiza la versión de Brain que incluye una versión nueva del SynapseSimulator, un re-seed lo aplica sin reinstalar Cortex. Eso es poderoso: el SynapseSimulator se actualiza con frecuencia durante el desarrollo y el mecanismo de actualización es simplemente `sentinel seed --profile-id {id} --reseed`.
...
**Paso 3 — El SynapseSimulator refleja el cambio automáticamente:** la próxima vez que se abre el SynapseSimulator, `ProtocolReader` carga el manifest actualizado y el Panel Simulate muestra el nuevo botón de `gemini_auth_complete`.

No hay paso 4. No hay cambio en el SynapseSimulator.
...
└── synapse_simulator_generator.py            ← NUEVO
...
**Los canales son tipos, no hardcoding.** El manifest diferencia mensajes de `runtime` y de `tabs`. El SynapseSimulator selecciona el mecanismo de dispatch correcto según el tipo. Nuevos tipos de canal se agregan al manifest sin tocar el SynapseSimulator.

**Dev/prod por construcción, no por flags.** El SynapseSimulator existe en builds de dev porque Brain lo genera en seed. No existe en prod porque Brain no lo genera. No hay flags de feature, no hay builds separados de Cortex.

**Re-seed como mecanismo de actualización.** Cambios en el SynapseSimulator se aplican con un re-seed. No requieren empaquetar ni distribuir Cortex.

---

*Documento generado como revisión y extensión de INVESTIGACION_Synapse_SynapseSimulator_Debug_Page.md. Este documento lo reemplaza en los puntos que refina.*
