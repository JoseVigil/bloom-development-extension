# SynapseSimulator — Manual de Uso y Debug del Protocolo Synapse (Cortex)

**Sistema:** Bloom Cortex · extensión Chrome MV3
**Versión del manual:** Junio 2026 — revisión Jun 25 (auditado contra código fuente)
**Propósito:** Contexto completo para retomar la investigación del SynapseSimulator en una nueva sesión.

---

## 1. Qué es el SynapseSimulator y para qué existe

El SynapseSimulator es una herramienta de observabilidad y simulación del protocolo Synapse. **Solo existe en builds dev** — no se despliega en producción.
...
## 2. Cómo se genera el SynapseSimulator

```
sentinel seed <alias> <is_master> --dev
```

El flag `--dev` pasa `--dev` a Brain, que llama a `synapse_simulator_generator.py`. Este copia los assets estáticos al directorio de extensión del perfil.

El archivo de configuración `synapse-simulator.synapse.config.js` **no se genera en seed** — se genera en cada launch por `ignition_identity.go::writeSynapseSimulatorConfig()`. Detecta dev mode chequeando si `synapse-simulator/index.html` existe en el extensionDir.

---

## 3. Prerequisitos para que el SynapseSimulator esté vivo
...
Para abrir Dev Tools del propio SynapseSimulator:
```
chrome://extensions → Bloom Nucleus Bridge → Inspect views → synapse-simulator/index.html
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  🌱 Bloom SynapseSimulator  [DEV]         MasterWorker  ● Config loaded  │  ← Top bar
├────────────────┬──────────────────────────────┬─────────────────┤
│                │                              │  [Log] [Config] │
│  PROTOCOLS     │  SIMULATE                    │                 │
...
- **Campos `type: auto`** — se resuelven automáticamente desde `SYNAPSE_SIMULATOR_CONFIG` (profileId) y `SYNAPSE_CONFIG` (launchId). No son editables.
...
| Tipo | Qué significa |
|---|---|
| `[INFO]` | Ciclo de vida del SynapseSimulator (boot, config loaded, synapse-simulator ready) |
...
**Replay automático al abrir:** gracias al mecanismo `SYNAPSE_SIMULATOR_HELLO`/`SYNAPSE_SIMULATOR_REPLAY`, cuando el SynapseSimulator abre envía automáticamente `SYNAPSE_SIMULATOR_HELLO` a background.js y recibe todos los eventos acumulados en el buffer circular (últimas 100 entradas). Esto permite ver `HANDSHAKE_CONFIRMED`, `EXTENSION_LOADED` y otros eventos que ocurrieron antes de que la tab del SynapseSimulator abriera.

### Tab Config

Muestra el estado de `SYNAPSE_SIMULATOR_CONFIG` y `SYNAPSE_CONFIG` cargados al boot.

Si `SYNAPSE_SIMULATOR_CONFIG` muestra `—` en todos los campos: `synapse-simulator.synapse.config.js` no existe todavía (el launch nunca corrió).
...
1. **POST** `http://localhost:48215/api/internal/system-event` → nucleus Control Plane → Workspace SynapseSimulator (Electron)
2. **`chrome.runtime.sendMessage({event: 'SYNAPSE_SIMULATOR_LOG', ...})`** → Cortex SynapseSimulator tab (este panel Log)

Ambos son fire-and-forget. El evento también se guarda en un buffer en memoria en background.js. El SynapseSimulator lo recupera vía `SYNAPSE_SIMULATOR_HELLO`/`SYNAPSE_SIMULATOR_REPLAY` al abrir.

Esto significa que el feed del Cortex SynapseSimulator y el feed del Workspace SynapseSimulator muestran los mismos eventos — son dos vistas del mismo bus.
...
> **Advertencia:** simular `host_ready` desde el SynapseSimulator cuando ya hay un host real conectado puede causar comportamiento inesperado. Usar solo cuando no hay host corriendo.

---

## 10. Arquitectura de archivos del SynapseSimulator

```
extension/
├── synapse-simulator.synapse.config.js        ← generado por Sentinel en cada launch
│                                       self.SYNAPSE_SIMULATOR_CONFIG = { profileId, launchId, profileAlias }
├── discovery.synapse.config.js      ← generado por Sentinel en cada launch
│                                       self.SYNAPSE_CONFIG = { profileId, launchId, ... }
└── synapse-simulator/
    ├── index.html                   ← layout de 3 paneles, solo carga <script src="synapse-simulator.js">
    └── synapse-simulator.js                   ← ProtocolReader, Simulator, Logger, ConfigReader, boot

extension/discovery/
├── discoveryProtocol.js             ← self.DISCOVERY_PROTOCOL_MANIFEST (8 mensajes)
└── synapseSimulatorProtocol.js              ← self.SYNAPSE_SIMULATOR_PROTOCOL_MANIFEST (10 comandos)
...
**Secuencia de boot de synapse-simulator.js (DOMContentLoaded):**
```
...
| `synapse-simulator.js` | Extensión Chrome (`chrome-extension://...`) | ✅ Sí |
...
- `synapse-simulator.js` — ProtocolReader (objeto literal con `discover()` legacy + `discoverFromJSON()` nuevo estándar), Simulator, Logger, ConfigReader, boot async
...
- `extension/protocols/` — `discovery.schema.json`, `landing.schema.json`, `synapse-simulator.schema.json` *(Fase 1–5)*

### Pendiente / No auditado

| Tarea | Archivo | Estado |
|---|---|---|
| Auditar | `nucleus/internal/supervisor/onboarding_synapse_simulator.go` | No provisto — auditar en próxima sesión |
| Verificar | `synapse-simulator/index.html` | No provisto — verificar que no tiene JS inline |
| Verificar | `synapseSimulatorProtocol.js` | No provisto en esta sesión (auditado Jun 19) |
...
1. **¿Qué contiene `nucleus/internal/supervisor/onboarding_synapse_simulator.go`?**
...
3. **¿El Workspace SynapseSimulator (Electron) comparte el formato de `SYNAPSE_SIMULATOR_LOG` con el Cortex SynapseSimulator?**
...
|---|---|---|
| Este documento | `/docs/SYNAPSE_SIMULATOR/` | Contexto completo de la sesión Jun 25 |
| `synapse-simulator/index.html` | `templates/synapse-simulator/index.html` | Verificar que no hay JS inline (CSP) |
| `synapseSimulatorProtocol.js` | `templates/synapse-simulator/synapseSimulatorProtocol.js` | Confirmar 10 mensajes vs doc |
| `onboarding_synapse_simulator.go` | `nucleus/internal/supervisor/onboarding_synapse_simulator.go` | Wildcard no documentado — auditar |
| `ignition_identity.go` | `sentinel/...` | Verificar `writeSynapseSimulatorConfig()` lines 408-444 |
...
*Basado en lectura directa de: background.js, synapse-simulator.js, discoveryProtocol.js, discovery.js, landingProtocol.js, landing.js*
