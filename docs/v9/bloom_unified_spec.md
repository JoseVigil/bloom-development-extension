...
Bloom utiliza VSCode Multi-Root Workspace para unificar todos los proyectos en una sola ventana:
...
- ✅ Compilación por proyecto individual (via terminal)
...
### 3.5 Creación de Nucleus con Workspace Unificado
...
**Caso 1: Repo no existe (Crear Nuevo)**
    ...
    4. Crea archivo {org}-workspace.code-workspace
    5. Inicializa workspace multi-root
    6. Ofrece abrir workspace (reemplaza ventana actual)

**Caso 2: Repo existe en GitHub (Clonar)**

    1. Plugin detecta existencia via API
    2. Clona en carpeta local elegida
    3. Crea workspace file si no existe
    4. Linkea en UserManager
...
**Caso 3: Repo existe local + GitHub (Linkear)**
    ...
    4. Crea workspace file si falta
...
**Flujo Mejorado:**
    ...
    5. Plugin ejecuta:
       ...
       f. Agrega proyecto al workspace actual
...
El plugin garantiza que cada proyecto hijo tenga estructura completa:
...
### 9.1 Plugin Tree
    ...
    │   │   ├── linkToNucleus.ts
    │   │   ├── manageProject.ts        ← NUEVO: Gestión completa
    │   │   └── [otros...]
    │   ├── core/
    │   │   ├── nucleusManager.ts
    │   │   ├── intentSession.ts
    ...
    │   ├── managers/
    │   │   ├── userManager.ts
    │   │   └── workspaceManager.ts    ← NUEVO: Multi-root workspace
    │   ├── models/
    │   │   ├── bloomConfig.ts
    │   │   └── intent.ts
    │   ├── providers/
...
**Archivos Nuevos (Workspace Feature):**
- `src/managers/workspaceManager.ts` - Gestión multi-root
- `src/commands/manageProject.ts` - CRUD completo de proyectos

**Archivos Modificados:**
- `src/providers/nucleusTreeProvider.ts` - Integración workspace
- `src/ui/welcome/welcomeView.ts` - Inicialización workspace
- `src/commands/linkToNucleus.ts` - Auto-add al workspace

### 9.2 Proyecto BTIP Completo con Workspace

**Organización en Disco:**

    /Users/jose/code/                    ← Parent folder
    ├── nucleus-josevigil/               ← Nucleus
    │   └── .bloom/
    │       ├── core/
    │       │   ├── nucleus-config.json
    │       │   ├── .rules.bl
    ...
    │               └── overview.bl
    │
    ├── bloom-video-server/              ← Proyecto hijo
    │   ├── .bloom/
    │   │   ├── core/                    ← Generado automáticamente
    │   │   │   ├── .rules.bl
    │   │   │   ├── .standards.bl
    │   │   │   └── .prompt.bl
    │   │   ├── project/                 ← Generado automáticamente
    │   │   │   ├── .context.bl
    │   │   │   └── .app-context.bl
    │   │   ├── intents/
    │   │   │   └── fix_login_bug/
    ...
    │   └── [código fuente...]
    │
    ├── bloom-mobile/                    ← Otro proyecto hijo
    │   ├── .bloom/
    │   │   └── [estructura similar...]
    │   └── [código fuente...]
    │
    └── josevigil-workspace.code-workspace  ← Workspace unificado
...
**Estructura .bloom/ en Proyecto Hijo:**

Generada automáticamente por `ensureBloomStructure()`:

    .bloom/
    ├── core/
    │   ├── .rules.bl              ← Template adaptado a estrategia
    │   ├── .standards.bl          ← Estándares de código
    │   └── .prompt.bl             ← Instrucciones para AI
    ├── project/
    │   ├── .context.bl            ← Contexto del proyecto
    │   └── .app-context.bl        ← Contexto específico de app
    ├── intents/                    ← Vacío al crear, se llena al usar
    └── nucleus.json               ← Link al Nucleus padre

**Verificación de Existencia:**

El plugin NO sobrescribe estructuras existentes:
- Si `.bloom/core` existe → Skip
- Si `.bloom/project` existe → Skip
...
Métodos principales:
    ...
    - initializeWorkspace(nucleusPath, orgName) → Promise<void>
    - addProjectToWorkspace(projectPath, projectName, strategy) → Promise<boolean>
    - removeProjectFromWorkspace(projectPath) → Promise<void>
    - syncWorkspaceFile() → Promise<void>
    - isMultiRootWorkspace() → boolean
    - getCurrentNucleusPath() → string | undefined
    - getProjectIcon(strategy) → string
...
**Ejemplo de uso:**
    ...
    // En openNucleusProject:
    const added = await WorkspaceManager.addProjectToWorkspace(
      projectPath,
      project.displayName,
      project.strategy
    );
...
Comportamiento mejorado:
    ...
    4. NO abre nueva ventana (eliminado vscode.openFolder(..., true))
...
- **Manejo de directorios faltantes:** Verifica existencia de .bloom/intents/ antes de leer

**Fix Crítico:**

Anterior: Crash si .bloom/intents/ no existe
Actual: Retorna array vacío y loggea warning
...
### 13.1 Flujo Completo: Desde Zero hasta Workspace Unificado
...
**Paso 2: Crear Primer Nucleus con Workspace**

    1. Activity Bar → Bloom icon
    2. Vista muestra: [+ Agregar Nucleus]
    3. Click "+" → Panel de setup
    4. Seleccionar organización (dropdown)
    5. Elegir parent folder (ej: ~/code/)
    6. Plugin ejecuta:
       a. Crea estructura nucleus-{org}/
       b. Ejecuta generate_nucleus.py
       c. Crea {org}-workspace.code-workspace
       d. Llama WorkspaceManager.initializeWorkspace()
    7. Plugin ofrece: "¿Abrir Workspace?"
    8. Usuario acepta → VSCode recarga con workspace
    9. Explorador muestra: 🏢 nucleus-josevigil

**Paso 3: Agregar Proyecto Existente al Workspace**

    1. Nucleus Tree → Click en proyecto vinculado
    2. Plugin ejecuta:
       a. Verifica si proyecto ya está en workspace
       b. Si no está → WorkspaceManager.addProjectToWorkspace()
       c. Asegura estructura .bloom/ completa
       d. Sincroniza .code-workspace file
    3. Proyecto aparece en explorador
    4. Enfoque automático en el proyecto
    5. NO se abre nueva ventana

**Paso 4: Clonar Nuevo Proyecto desde GitHub**

    1. Nucleus Tree → Click "+ Agregar Proyecto"
    2. Seleccionar "Clonar desde GitHub"
    3. Ingresar URL del repo
    4. Plugin ejecuta:
       a. Clona en parent folder (junto a Nucleus)
       b. Detecta estrategia automáticamente
       c. Ejecuta ensureBloomStructure()
       d. Crea nucleus.json con link
       e. Agrega LinkedProject a nucleus-config.json
       f. Genera overview.bl en Nucleus
       g. Actualiza _index.bl
       h. Agrega al workspace actual
    5. Proyecto clonado aparece en explorador
    6. Estructura .bloom/ lista para usar

**Paso 5: Crear Proyecto Nuevo desde Template**

    1. Nucleus Tree → Click "+ Agregar Proyecto"
    2. Seleccionar "Crear Nuevo Proyecto"
    3. Elegir estrategia (Android, Node, Python-Flask, etc.)
    4. Ingresar nombre del proyecto
    5. Plugin ejecuta:
       a. Crea carpeta en parent folder
       b. Genera estructura básica según estrategia
       c. Crea .bloom/ completa con templates
       d. Inicializa git (opcional)
       e. Agrega al workspace
       f. Vincula con Nucleus
    6. Proyecto nuevo listo para desarrollo

**Paso 6: Vincular Proyecto Existente Local**

    1. Nucleus Tree → Click "+ Agregar Proyecto"
    2. Seleccionar "Vincular Proyecto Local"
    3. Browse para seleccionar carpeta
    4. Plugin ejecuta:
       a. Verifica que no esté ya vinculado
       b. Detecta estrategia
       c. Asegura .bloom/ (respeta existente)
       d. Crea nucleus.json
       e. Vincula con Nucleus
       f. Agrega al workspace
    7. Proyecto integrado al workspace

**Paso 7: Crear Intent en Proyecto del Workspace**

    1. En explorador, navegar a bloom-video-server/
    2. Seleccionar archivos relevantes
    3. Click derecho → "Generate New Intent"
    4. Formulario se abre pre-llenado
    5. (Resto del flujo igual que documentado previamente)
    6. Intent se guarda en .bloom/intents/
    7. Funciona independientemente del workspace

**Paso 8: Navegar entre Proyectos**

    1. Explorador VSCode muestra todos los proyectos
    2. Click en cualquier archivo para editarlo
    3. Terminal integrada puede ejecutar comandos por proyecto
    4. Compilación manual por proyecto (si necesario)
    5. Búsqueda global funciona en todo el workspace

**Beneficios del Flujo:**
- ✅ Todo en una sola ventana VSCode
- ✅ Sin pérdida de contexto al cambiar proyectos
- ✅ Compilación no afectada (sigue siendo por proyecto)
- ✅ Persistencia del workspace entre sesiones
- ✅ Estructura .bloom/ garantizada en todos los proyectos
...
**Test 2: Crear Nucleus con Workspace**
    ...
    6. Verificar .code-workspace creado
    7. Verificar workspace tiene nombre correcto
    8. Abrir workspace y verificar Nucleus aparece
...
**Test 3: Agregar Proyecto al Workspace**
    ...
    6. Verificar folders[] contiene nuevo proyecto
    7. Click nuevamente → Verificar solo enfoca (no duplica)
...
**Test 4: Estructura .bloom/ Automática**
    ...
    4. Verificar templates .bl correctos por estrategia
    5. Verificar nucleus.json creado con link correcto
    6. Clonar proyecto CON .bloom/
    7. Verificar estructura existente NO sobrescrita
...
**Test 7: Link to Nucleus**
    ...
    8. Verificar proyecto agregado al workspace

**Test 8: Workspace Persistence**

    1. Crear workspace con 3 proyectos
    2. Cerrar VSCode
    3. Abrir .code-workspace file
    4. Verificar todos los proyectos presentes
    5. Verificar settings correctas
    6. Verificar iconos en nombres

**Test 9: IntentTreeProvider sin .bloom/intents/**

    1. Crear proyecto nuevo (sin intents/)
    2. Abrir Nucleus Tree
    3. Verificar NO hay crash
    4. Verificar log warning en output
    5. Verificar tree muestra vacío correctamente
...
Sistema Workspace:
    ...
    ✅ Compilación por proyecto NO afectada
...
Estructura .bloom/:
    ...
    ✅ ensureBloomStructure() crea estructura completa
    ✅ Respeta estructuras existentes (no sobrescribe)
    ✅ Templates adaptados a estrategia del proyecto
    ✅ nucleus.json creado con link correcto
    ✅ Directorios core/, project/, intents/ presentes
...
Integración:
    ...
    ✅ WorkspaceManager sincroniza estado correctamente
...
**Problema: Proyecto abre en nueva ventana en lugar de agregarse al workspace**
...
**Problema: Estructura .bloom/ incompleta en proyecto clonado**

Solución:
- Verificar ensureBloomStructure() ejecutado
- Check logs para errores de escritura
- Verificar permisos en carpeta del proyecto
- Ejecutar manualmente comando de vinculación

**Problema: TreeView crashea al abrir**

Solución:
- Verificar existencia de .bloom/intents/ en todos los proyectos
- Check que IntentTreeProvider maneja directorios faltantes
- Revisar logs para ENOENT errors
- Actualizar plugin a última versión con fix

**Problema: .code-workspace no se sincroniza**

Solución:
- Verificar WorkspaceManager.syncWorkspaceFile() ejecutado
- Check que archivo no está bloqueado por otro proceso
- Revisar permisos de escritura
- Reabrir workspace manualmente
...
**Problema: Workspace muestra "UNTITLED"**

Solución:
- Verificar nombre del archivo: {org}-workspace.code-workspace
- Check que settings.window.title está configurado
- Renombrar workspace file manualmente si necesario
...
**Fase 1 (Completado):**
    ...
    ✅ Navegación sin ventanas nuevas
...
**Fase 2 (Q1 2026):**
    ...
    🔲 Gestión avanzada de proyectos:
       - Crear proyecto desde template mejorado
       - Import bulk de repositorios
...
**Fase 3 (Q2 2026):**
    ...
    🔲 Comandos de terminal integrados por proyecto
...
**Performance:**
    ...
    - Debounce mejorado para sincronización de workspace
...
**UX:**
    ...
    - Indicadores visuales de estado de compilación
    - Quick actions en Nucleus Tree
    - Workspace switcher en status bar
...
**Integración:**
    ...
    - Webhooks para eventos de workspace
    - CI/CD integration hooks

**Workspace Enhancements:**

    - Auto-detección de proyectos en parent folder
    - Sugerencias inteligentes de vinculación
    - Reorganización de folders en workspace
    - Workspace templates por tipo de organización
...
5. **bloom-btip-resumen.md** - Workspace unificado y gestión de proyectos
...
- `src/managers/workspaceManager.ts` - Gestión multi-root workspace
...
**Multi-Root Workspace:** Workspace de VSCode que contiene múltiples carpetas de proyectos en una sola ventana.

**WorkspaceManager:** Componente que gestiona la creación y sincronización de workspaces multi-root.

**ensureBloomStructure():** Función que garantiza la existencia de estructura .bloom/ completa en proyectos.

**LinkedProject:** Proyecto técnico vinculado a un Nucleus (registrado en nucleus-config.json).

**Parent Folder:** Carpeta contenedora donde residen Nucleus y proyectos relacionados.

**.code-workspace:** Archivo JSON de VSCode que define la configuración de un multi-root workspace.
...
Bloom es un plugin de VSCode que unifica tres sistemas poderosos:
...
3. **Workspace Unificado** - Para navegación fluida sin ventanas múltiples
...
- **Workspace multi-root nativo** con sincronización automática
- **Estructura .bloom/ garantizada** en todos los proyectos
- **Navegación unificada** sin pérdida de contexto

### Características Destacadas del Workspace

El sistema de workspace unificado representa un avance significativo:

**Beneficios Técnicos:**
- Usa APIs nativas de VSCode (workspace.updateWorkspaceFolders)
- Persistencia automática entre sesiones
- No interfiere con compilación por proyecto
- Sincronización bidireccional con archivo .code-workspace

**Beneficios UX:**
- Todo en una sola ventana VSCode
- Click en proyecto = enfoque automático (sin ventanas nuevas)
- Búsqueda global en todo el workspace
- Terminal integrada por proyecto
- Estructura .bloom/ siempre completa y válida

**Escalabilidad:**
- Funciona desde 1 hasta N proyectos
- No hay límite de proyectos en workspace
- Detección automática de proyectos hermanos
- Templates adaptados por estrategia

El sistema está diseñado para escalar desde desarrolladores individuales hasta organizaciones con múltiples proyectos y equipos, manteniendo siempre la coherencia de estructura y la fluidez de navegación.

### Próximos Pasos Recomendados

1. **Prioridad Alta:** Testing completo del workspace (casos edge, rendimiento)
2. **Prioridad Media:** Debugging de intents (workflow completo con Claude)
3. **Prioridad Baja:** Features avanzadas (templates, analytics, CI/CD)
...
## Apéndice A: Problemas Resueltos (Workspace Feature)

### A.1 Problema Principal Solucionado

**Antes:**
- Click en proyecto → Nueva ventana VSCode
- Contexto perdido al cambiar proyectos
- Sin persistencia de workspace
- Proyectos clonados sin estructura .bloom/

**Después:**
- Click en proyecto → Agregado al workspace actual
- Todo en una ventana unificada
- Workspace persiste automáticamente
- Estructura .bloom/ garantizada siempre

### A.2 Fixes Implementados

1. **openNucleusProject():** Reemplazado `vscode.openFolder(..., true)` por `WorkspaceManager.addProjectToWorkspace()`
2. **ensureBloomStructure():** Nueva función que crea estructura completa respetando existente
3. **IntentTreeProvider:** Manejo de directorio .bloom/intents/ faltante (no crashea)
4. **Workspace File:** Generación automática con nombre correcto y settings apropiadas
5. **Sincronización:** WorkspaceManager.syncWorkspaceFile() mantiene estado consistente

### A.3 Arquitectura del Fix

    Usuario hace click en proyecto
            ↓
    nucleusTreeProvider.openNucleusProject()
            ↓
    WorkspaceManager.addProjectToWorkspace()
            ↓
    1. Verifica si ya está en workspace
    2. Si no está → updateWorkspaceFolders()
    3. ensureBloomStructure() si falta
    4. syncWorkspaceFile()
    5. revealInExplorer()
            ↓
    Proyecto visible en explorador
    (Sin nueva ventana)

---

*Para más información técnica detallada, consultar codebase.md con el código fuente completo.*
*Para historial de desarrollo del workspace, consultar bloom-btip-resumen.md.*