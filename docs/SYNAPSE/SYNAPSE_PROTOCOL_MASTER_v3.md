...
### Versión: 3.0 — Actualizado con arquitectura Electron Bridge, secuencia de boot canónica y flujo completo Workspace→Cortex
...
**v3.0 — Arquitectura Electron Bridge + Secuencia de Boot Canónica**
- Sección 2 actualizada: flujo completo `Workspace → nucleus → Sentinel → Brain → Host → Cortex` documentado como ley
- Nueva sección 2A: Electron Bridge (SynapseBridge, preload-synapse.js, clasificación de eventos)
- Nueva sección 2B: Secuencia de boot canónica con Temporal (`temporal ensure → worker start → seed → launch`)
- Sección 7 actualizada: gap de integración Workspace↔Sentinel aclarado — WS nucleus `:4124` es observabilidad, no canal de protocolo
- Tabla de archivos clave (Sección 9) actualizada con archivos del bridge Electron
...
### El flujo completo — ley inalterable del sistema

```
Workspace → nucleus → Sentinel → Brain → Host → Cortex
         ←          ←          ←       ←      ←
```

Este flujo es **bidireccional e inalterable**. No hay atajos ni conexiones directas entre
capas no adyacentes. Cada componente solo conoce a sus vecinos inmediatos:

- **Workspace** habla con **nucleus** (HTTP POST `:48215`, WS `:4124` para observabilidad)
- **nucleus** orquesta via **Temporal** → **Sentinel** (binario Go)
- **Sentinel** habla con **Brain** via TCP `:5678` (protocolo 4-byte BigEndian)
- **Brain** habla con **Host** via IPC TCP localhost (port file en `run/ipc_{launch_id}.port`)
- **Host** habla con **Cortex** via Chrome Native Messaging (`stdin/stdout`, 4-byte LE + JSON)

> ⚠️ **El WS de nucleus (`:4124`) es un canal de observabilidad del debug panel — no es
> el canal de protocolo.** Los eventos que Workspace dispara al bus de nucleus son
> recibidos por el debug panel (Workspace SynapseSimulator). Sentinel los recibe a través de
> Temporal workflows, no escuchando el WS directamente.
...
## 2A. ELECTRON BRIDGE — CÓMO WORKSPACE RECIBE EVENTOS DE BRAIN

El main process de Electron es donde corren el spawn de `nucleus` y el socket TCP a Brain.
El renderer (UI de workspace/onboarding) vive en contexto aislado (`contextIsolation: true`).
`SynapseBridge` es el componente que atraviesa ese aislamiento.

### Arquitectura del bridge

```
Brain (TCP IPC, 4-byte UInt32 LE + JSON)
    ↓
SynapseBridge (main process — conductor/shared/synapse-bridge.js)
    ↓  webContents.send('synapse:event', payload)
preload-synapse.js (contextBridge)
    ↓  window.bloomSynapse.onEvent(callback)
Renderer (UI — workspace/onboarding)
```
...
|---|---|
| `conductor/shared/synapse-bridge.js` | Motor del protocolo: spawn nucleus, polling port file, conexión TCP, parseo 4-byte LE, clasificación de mensajes, watchdog heartbeat |
| `conductor/shared/preload-synapse.js` | Expone `window.bloomSynapse` al renderer via `contextBridge` |
| `conductor/setup/ipc/setup-synapse-handlers.js` | `ipcMain.handle` para la app de instalación (singleton) |
| `conductor/workspace/ipc/workspace-synapse-handlers.js` | `ipcMain.handle` para workspace (Map por `webContentsId`, soporta múltiples ventanas) |

### Clasificación de eventos que llegan al renderer

Todos los eventos que llegan a `window.bloomSynapse.onEvent(callback)` tienen la forma:

```javascript
{
  type:       string,   // clasificación — ver tabla
  _ts:        number,   // Date.now() cuando llegó al main process
  _profileId: string,
  _launchId:  string,
  // ...campos del payload JSON de Brain (sin modificar)
}
```

| `type` | Origen | Cuándo |
|---|---|---|
| `STATUS` | Bridge (sintético) | Cambios de fase interna del bridge |
| `HEARTBEAT` | Brain | Señal periódica de perfil vivo |
| `HANDSHAKE` | Brain | `event: 'HANDSHAKE_CONFIRMED'` — handshake 3 fases completo |
| `HOST_READY` | Brain | `event: 'HOST_READY'` |
...
| `SYNAPSE_EVENT` | Brain | Catch-all para eventos no clasificados |
...
const unsubscribe = window.bloomSynapse.onEvent(event => {
  switch (event.type) {
    case 'HANDSHAKE':   showHandshakeConfirmed(); break;
    case 'HEARTBEAT':   updateHeartbeatIndicator(); break;
    case 'INTENT':      updateIntentProgress(event); break;
    case 'SYNAPSE_EVENT': console.log('Brain event:', event.command || event.event); break;
  }
});
window.addEventListener('beforeunload', () => unsubscribe());
```

### Troubleshooting del bridge

| Síntoma | Causa | Fix |
|---|---|---|
| `window.bloomSynapse` es `undefined` | Preload no configurado para esa ventana | Verificar que `preload:` en `webPreferences` apunta al archivo correcto |
| Port file no aparece | Brain no levantó el IPC server | Revisar logs en `BloomNucleus/logs/`; el bridge espera 30s |
| Heartbeat llega pero HANDSHAKE no | Handshake iniciado por `background.js` de Cortex, no por el bridge | Verificar Cortex/background.js — el bridge solo escucha pasivamente el `HANDSHAKE_CONFIRMED` |
| `nucleus: command not found` | `PATH` difiere en Electron empaquetado | Usar `getBloomRoot()` para derivar el path absoluto del binario |

> **Nota crítica:** El bridge **no necesita modificarse** para absorber mensajes nuevos de Brain.
> Cualquier mensaje no clasificado llega al renderer como `SYNAPSE_EVENT` con todos sus campos.
> Para promover un tipo nuevo a categoría propia, agregar un `case` en `_classifyMessage()` en `synapse-bridge.js`.

---

## 2B. SECUENCIA DE BOOT CANÓNICA
...
# NO usar: nucleus temporal start  (deprecado para automation — modo interactivo)

# 2. Worker — OBLIGATORIO (sin Worker los workflows quedan en pausa indefinida)
nucleus worker start -q profile-orchestration

# 3. Seed — crear identidad persistente del perfil en Temporal
nucleus synapse seed <alias> --master

# 4. Launch — iniciar Chrome + Sentinel
...
```bash
nucleus dev-start
# Equivalente a: temporal ensure + worker start + configuración dev
```

### Flags globales — regla crítica

Los flags `--json` y `--verbose` van **después de `nucleus` y ANTES del subcomando**:
...
### Respuesta JSON de launch (éxito)
...
### Actividades Sentinel — unificadas en v3.0

Todas las Sentinel Activities están consolidadas en:
```
internal/orchestration/activities/sentinel_activities.go
```
No existe `internal/synapse/` — esa rama fue eliminada en el refactor de Feb 2026.
El patrón de registro de nuevos comandos usa `createXxxSubcommand` y `cmd.AddCommand()` dentro del padre.
...
### Aclaración de arquitectura — WS nucleus vs canal de protocolo

El `EventBus` de Brain (TCP `:5678`) y el bus WS de nucleus (`:4124`) son **dos buses
separados con propósitos distintos**:

- **WS nucleus `:4124`** — observabilidad del debug panel (Workspace SynapseSimulator). Los eventos
  que Workspace dispara aquí los consume `debug.html`. No son canal de protocolo.
- **TCP Brain `:5678`** — canal de protocolo real. Sentinel se conecta aquí via `EventBus.go`
  y hace `REGISTER_SENTINEL`. Los mensajes de hito viajan por este canal.

**Inventario completo de handlers TCP en `ServerManager` (Brain):**

| msg_type | Handler | Output |
|---|---|---|
| `REGISTER_CLI` / `REGISTER_SENTINEL` | Registra Sentinel | `REGISTER_ACK` |
| `REGISTER_HOST` | Registra bloom-host, marca perfil online | `REGISTER_ACK` |
| `PROFILE_CONNECTED` | Confirma handshake, emite al EventBus | broadcast `PROFILE_CONNECTED` |
| `HEARTBEAT` | Actualiza timestamp | — |
| `POLL_EVENTS` | Devuelve eventos desde timestamp | `EVENTS` |
| `GET_PROFILE_STATE` | Devuelve estado del perfil | `PROFILE_STATE` |
| `PROFILE_CREATE` | Crea perfil nuevo | `PROFILE_CREATE_ACK` |
| `LAUNCH_PROFILE` | Lanza Chrome con spec_path | `LAUNCH_PROFILE_ACK` + broadcast `PROFILE_LAUNCHED` |
| `HOST_INIT` | Inicializa host Synapse | `HOST_INIT_ACK` |
| `GITHUB_TOKEN_STORED` | Persiste fingerprint PAT, avanza onboarding | `GITHUB_TOKEN_STORED_ACK` + broadcast `ONBOARDING_STEP_COMPLETE` |
| `UNREGISTER_HOST` | Cierra conexión limpiamente | break + cleanup |

**Hitos faltantes en `ServerManager` (sin handler explícito todavía):**
- `API_KEY_REGISTERED` — análogo a `GITHUB_TOKEN_STORED` para el paso 2 del onboarding
- `DISCOVERY_COMPLETE` — cierre del flujo de onboarding
...
| `conductor/shared/synapse-bridge.js` | Bridge Electron main↔Brain: TCP, clasificación, watchdog | Si cambia el wire format o se agrega categoría de evento |
| `conductor/shared/preload-synapse.js` | contextBridge renderer←→main | Si se agrega API nueva al renderer |
| `conductor/setup/ipc/setup-synapse-handlers.js` | ipcMain handlers para setup | Si se agrega comando desde setup UI |
| `conductor/workspace/ipc/workspace-synapse-handlers.js` | ipcMain handlers para workspace | Si se agrega comando desde workspace UI |
| `internal/orchestration/activities/sentinel_activities.go` | Todas las Sentinel Activities (unificadas v3.0) | Si se agrega activity de orquestación |

---

*Versión 3.0 — Arquitectura Electron Bridge + Secuencia de Boot Canónica + Flujo completo Workspace→Cortex documentado.*
*Próxima actualización: cuando Tests 1-5 se completen en Darwin, o cuando se implementen handlers `API_KEY_REGISTERED` y `DISCOVERY_COMPLETE` en ServerManager.*
