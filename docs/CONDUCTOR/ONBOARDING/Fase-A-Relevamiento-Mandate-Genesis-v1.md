...
Más aún: `_onMandateGenesisComplete()` en la práctica **nunca se ejecuta** en el camino normal, porque nada llama a `reactor.handleMilestone('mandate_genesis', ...)` — `onboarding:create-mandate` persiste todo directo a `nucleus.json` sin pasar por el reactor (ver A.1). Es código correcto pero muerto salvo que algo lo dispare manualmente (harness, o un futuro evento real de Brain que hoy no existe para este step).
