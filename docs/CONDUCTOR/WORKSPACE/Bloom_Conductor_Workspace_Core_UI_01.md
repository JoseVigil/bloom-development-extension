...
**D-04 marcado como resuelto.** `workspace_url` ya no está hardcodeado a `:3000`; la deuda técnica correspondiente queda cerrada.
...
| ~~D-04~~ | ~~`workspace_url` hardcodeado a `:3000`~~ **[RESUELTO]** | Ya no aplica — `workspace_url` dejó de estar hardcodeado |
