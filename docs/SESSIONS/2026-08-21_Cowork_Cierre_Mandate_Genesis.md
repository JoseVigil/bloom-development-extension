...
3. **Migración de UI de Core** — retomar donde quedó: pasar el screenshot del estado actual de `/home` para decidir, de a partes, qué se remueve/ajusta primero contra el diseño objetivo. Ojo con la discrepancia ya detectada entre el vocabulario de eventos `mandate:*` que se cableó en esta sesión (~10 eventos, `ws-events.ts`) y los 6 eventos reales que documenta el mock (`mandate:phase:ingest`, `mandate:action:started/completed/failed`, `mandate:build:rejected`, `mandate:build:all_complete`) — son vocabularios distintos, hay que reconciliarlos antes de dar por buena la implementación de `mandateStore.applyMandateEvent()`.
