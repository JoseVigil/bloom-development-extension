...
**v3.3 — Fase B implementada (Cowork) sobre v3.2/v3.1 (Pivot v1.4), alineado con `ING_Intent_Spec_v1_1.md` y `DIS_Intent_Spec_v1_0.md`**
...
**Qué agregó v3.2:** el relevamiento de Fase A (Cowork) sobre código real, que corrigió una asunción central de v3.1 (el disparador del cierre de Onboarding no es `milestone-reactor.js`, es el IPC `onboarding:complete`) y cerró el diseño de D-22/D-23 con una decisión concreta, todavía sin implementar en ese momento.

**Qué agrega v3.3:** la implementación real de esa decisión (Fase B, Cowork) para D-22 (contrato del step) y D-23 (flag `pending_genesis_launch`), más un hallazgo nuevo no anticipado por ninguna versión anterior — el step `mandate_genesis` está declarado en **tres** lugares del código (`onboarding_steps.json`, y dos copias de `FALLBACK_STEPS` en `milestone-registry.js` y `navigation.js`), no en uno solo — registrado como D-27. **QA manual todavía pendiente** — ver §1.2 para el detalle de qué falta probarse a mano antes de dar esto por cerrado de verdad.
...
- **✅ Confirmado (código real — GAP V3):** verificado línea por línea contra `mandate_genesis_activities.go`, `mandate_build_workflow.go`, `mandate_watcher.go`, `ws-events.ts`.
...
**Nota v3.1:** "Fases (1-4)" sigue siendo un eje válido para describir el ciclo de vida de un Genesis Mandate — pero bajo el pivot de esta versión deja de ser sinónimo de "los 4 pasos hardcodeados de un `MandateBuildWorkflow` dedicado" (ver §2.1). Pasa a describir el mismo ciclo lógico (ingest → cluster → validate → scaffold) ejecutado por el motor genérico de Mandate, sin Workflow propio de Genesis. El eje no desaparece, cambia lo que hay por debajo.
...
**Qué existía hasta ahora:** Genesis era un tipo de Mandate con tratamiento especial de punta a punta — su propio Workflow en Go (`mandate_build_workflow.go`), sus propias Activities (`mandate_genesis_activities.go`), su propio canal de Signal "brandeado" (`mandate:build:validate`, `mandate:build:rejected`, `mandate:build:all_complete`), y una función de firma (`SignMandateActivity`) que hardcodeaba el string `"genesis"` sin mirar el `MandateType` real. Aunque el dato (`MandateType: "genesis" | "domain_expansion"`) ya era genérico en el modelo, el *código* seguía bifurcando: un camino "Genesis", y todo lo demás tratado como caso secundario mal integrado.
...
**Estado real de esto:** decisión de arquitectura recién tomada, no implementada — el refactor de `mandate_build_workflow.go`/`mandate_genesis_activities.go` hacia un motor genérico todavía no se hizo (se había pausado antes por esta misma razón: no tenía sentido renombrar el archivo mientras el contenido seguía siendo Genesis-específico). Punto de acción concreto para UI: antes de invertir esfuerzo separando `GenesisTab` de `StandardMandateTab` como venía el roadmap de v3, confirmar si esa separación sigue siendo necesaria bajo esta arquitectura, o si el Paso 4 se simplifica a "una sola tab de Mandate, genérica, con variaciones de estado según `MandateType`" (ver §4).
...
CORE (Electron, ventana 2 — Svelte webview en :5173)
  └── mandate_watcher.go (servicio Go, ya corriendo en paralelo) detecta el archivo por fsnotify
        └── arranca MandateBuildWorkflow (Temporal) — YA CORRIÓ o está por correr
              │
              ▼
...
### 1.2 Mecanismo de paso de parámetros Onboarding → Core

**Actualización v3.2 — corrección confirmada por código real (Fase A, Cowork):** el párrafo original de v3.1 acá abajo asumía que `conductor_reaction: onOnboardingSuccess` (definido en `milestone-registry.js`, reaccionado en `milestone-reactor.js`) era el mecanismo que dispara el cierre de la ventana de Onboarding. **No lo es.** Confirmado contra `onboarding-handlers.js`, `step-milestone.js` y `milestone-reactor.js`: `_onOnboardingSuccess()` solo emite un evento `milestone:reached` al renderer y notifica a Discovery/Chrome vía `nucleus synapse onboarding` — no abre ni cierra ninguna ventana de Electron. El disparador real es otro, completamente separado:
...
step-milestone.js (pantalla de milestone, ya con el mandate creado)
  → usuario hace click "Enter System" → completeOnboarding()
  → IPC onboarding:complete → onboarding-handlers.js
  → createWorkspaceWindow(url) + cierre de la ventana de Onboarding vieja   [ACÁ es el disparador real]
```

Esto importa porque cualquier rediseño de este step (abajo) tiene que engancharse en el IPC `onboarding:complete`, no en `milestone-reactor.js` — enganchar en el lugar equivocado sería construir sobre un mecanismo que hoy es, en la práctica, código muerto para este flujo.

**Decisión de diseño cerrada (v3.2) para D-22 y D-23** — reemplaza el "hace falta redefinir" de v3.1 por una decisión concreta, todavía sin implementar:

- **D-22 — contrato del step redefinido:** se mantiene el mismo mecanismo `verify: json_field` que ya usa el resto de `onboarding_steps.json`/`milestone-registry.js` (no hace falta un tipo de verify nuevo). Campo candidato: `onboarding.mandate_screen_acknowledged` (booleano), producido cuando el usuario reconoce la pantalla explicativa — mismo patrón exacto que hoy usa `genesis_mandate_id`, solo que apuntando a un campo que no depende de que el mandate ya exista.
- **D-23 — canal de paso de parámetros:** confirmado contra `main_conductor.js` que **hoy no existe ningún canal** — `createWorkspaceWindow(url)` recibe un único parámetro (la URL) y Core se abre completamente en blanco, sin query string, sin `additionalArguments` en `webPreferences`, sin variable de entorno. No hay nada que reusar; hay que diseñar uno. La decisión: un flag persistido en `nucleus.json` — `onboarding.pending_genesis_launch` (con `project`/`projectPath` adentro) — es el mecanismo elegido, por ser consistente con el patrón que ya domina el resto de este código (`workspace_path_pending`, `ownership_init_status`: flags compartidos entre procesos vía `nucleus.json`, no canales de Electron ad hoc). Dos reglas de implementación no negociables para que esto no rompa el flujo normal: (a) lo escribe el handler de `onboarding:complete`, justo antes de `createWorkspaceWindow(...)` — mismo punto donde hoy ya escribe `onboarding.completed: true`; (b) Core lo **consume y borra** al bootear, no solo lo lee — si no se limpia, cada apertura posterior de Core (sin venir de un Onboarding recién cerrado) volvería a intentar arrancar Genesis.

**Nota adicional de Fase A, relevante para B.2/B.4 de la directiva de Cowork:** `core/preload_core.js` ya expone, hoy, el mismo shape de `window.onboarding` que usa Onboarding (`createMandate`, `complete`, `markStepComplete`) — y `registerOnboardingHandlers(...)` ya se registra en ambas ramas de `app.whenReady()` en `main_conductor.js`. Esto significa que el caso "a demanda" (crear un mandate Genesis desde Core en cualquier momento, no solo al abrir tras Onboarding) **ya es técnicamente alcanzable hoy**, sin cambios de Electron/preload — la pregunta pendiente no es "¿se puede desde Core?" sino si conviene reorganizar el handler bajo `core/ipc/` por prolijidad, y eso puede esperar a que el Paso 4 de UI (§4, D-25) defina dónde aterriza el botón que lo dispara, en vez de decidirse en el vacío ahora.

---

**Texto original v3.1 (referencia, contenía la asunción ya corregida arriba):** el contrato de datos del step `mandate_genesis` en `onboarding_steps.json` cambia: hoy produce `genesis_mandate_id` y lo verifica contra ese campo (`verify: json_field`). Si la creación real se difiere a Core, ese step deja de poder verificar algo que todavía no existe en ese momento — hace falta redefinir qué produce y verifica en su lugar.

---

**Actualización v3.3 — implementado (Fase B, Cowork), QA manual pendiente.** El diseño de arriba ya está en código, no solo en papel:

- **D-22 (implementado):** `step-mandate.js` ya no llama `window.onboarding.createMandate(...)`. El botón dispara `window.onboarding.markStepComplete({step: 'mandate_genesis'})` — mecanismo genérico ya existente, sin IPC nuevo. `produces`/`verify` cambiaron de `genesis_mandate_id` a `mandate_screen_acknowledged` (mismo `verify: json_field`). Copy de `onboarding.html` actualizado acorde.
- **D-23 (implementado):** `onboarding-handlers.js` resuelve el proyecto activo y escribe `onboarding.pending_genesis_launch` en `nucleus.json` dentro del handler de `onboarding:complete`, antes de `createWorkspaceWindow(...)`. Se agregó un handler nuevo, `onboarding:consume-pending-genesis-launch` — lee **y borra** el flag atómicamente (no una lectura simple), expuesto en `core/preload_core.js`.
- **B.4.1 (implementado):** hook nuevo `webview/app/src/lib/bootstrap/genesisLaunch.ts`, enganchado en `+layout.svelte` — al bootear Core, consume el flag y si viene presente dispara la creación real del mandate. **Deliberadamente no conectado a `MandateTab`/`tabsStore` todavía** — con D-25 sin cerrar no hay dónde aterrizar esa UI; queda como hook preparado con TODO explícito, tal como pedía B.4 de la directiva.
- **B.4.2 / D-26: sin tocar**, tal como se decidió — el handler de creación de mandate sigue donde estaba, ya alcanzable desde Core sin cambios de Electron.

**Hallazgo nuevo de esta implementación, no anticipado por v3.1/v3.2 — ver D-27 (§6):** el step `mandate_genesis` no vive en un único SSOT. Está declarado en `onboarding_steps.json`, y **además** en dos copias de `FALLBACK_STEPS` — una en `milestone-registry.js`, otra embebida en `navigation.js`. Cowork tuvo que sincronizar los tres para este cambio. Quedaron sincronizados hoy, pero el riesgo de que una edición futura toque solo uno de los tres y deje los otros desactualizados sigue latente — no es parte del alcance de esta sesión, se registra para no repetir el rastreo la próxima vez.

**QA manual pendiente antes de cerrar D-22/D-23 del todo (nadie las corrió todavía, ni Cowork ni nosotros):**
1. Correr el onboarding completo una vez y confirmar que, al llegar a Core, el mandate se crea con el proyecto real (no un placeholder o un `local-${Date.now()}` de fallback — ver la nota de A.1 sobre el shape de stdout no confirmado del binario `nucleus`).
2. Cerrar Core y volver a abrirlo sin pasar por Onboarding — confirmar que **no** vuelve a intentar crear un mandate (valida que el borrado del flag funciona en la práctica, no solo que compila).
3. Inspeccionar `nucleus.json` durante el proceso para confirmar que `pending_genesis_launch` aparece y después desaparece.
...
### 1.1 Fase 0 — Setup / `MandateBuildInput` (Temporal) — ✅ Confirmado (código real — GAP V3)

La firma real del input de Temporal **no incluye `RawDocs` ni empaquetado de adjuntos**. El struct `MandateBuildInput` tiene únicamente estos campos, confirmados contra `mandate_build_workflow.go`:
...
**Qué cambia respecto a la tabla de §2:** deja de existir un `MandateBuildWorkflow` (Temporal/Go) dedicado a Genesis, con sus propias Activities y su propio canal de Signal. En su lugar, **todo Mandate — sin importar `MandateType`** — se ejecuta con el mismo motor genérico: un Mandate descompone su trabajo en Actions, y cada Action se resuelve como uno de los Intents estándar del sistema (`ing/`, `dis/`, y los ya existentes `dev/`, `doc/`, `exp/`, `inf/`; `cor/` también existía acá, pero fue deprecado 2026-09-02 por control, sin transición en curso, ver `docs/CONTROL/AGENDA_MAESTRA.md` §11). Esto es la aplicación, un nivel más arriba, del mismo principio que ya se aplicó a nivel Intent al reemplazar `gen/` por `ing/` — ver §0.1, CAMBIO BACKEND, para el razonamiento completo.

**Cómo se ve un Mandate Genesis bajo este motor:**

| Concepto viejo (v3, Workflow dedicado) | Concepto nuevo (v3.1, motor genérico) |
|---|---|
| `MandateType: "genesis"` dispara `MandateBuildWorkflow` | `MandateType: "genesis"` es solo metadata de negocio — el Mandate corre en el mismo motor que cualquier otro |
...
| Fase 3 (validate) = Signal `mandate:build:validate` "brandeado" | Turno de `.consolidation/` dentro de ese mismo `ing/` — Signal genérico de Mandate, no un canal exclusivo de Genesis |
...
**Estado:** diseño recién tomado, no implementado. El refactor de `mandate_build_workflow.go`/`mandate_genesis_activities.go` hacia el motor genérico todavía no arrancó — venía pausado desde antes de este pivot, precisamente porque no tenía sentido tocar esos archivos mientras seguían siendo Genesis-específicos de punta a punta.
...
- `mandate:build:rejected`
- `mandate:build:all_complete`
...
| **D-24 (nuevo, v3.1)** | Eliminación de `MandateBuildWorkflow`/`mandate_genesis_activities.go` como camino especial, y del hardcode de `"genesis"` en `SignMandateActivity` | Backend | **🔀 Diseño tomado (§2.1), refactor no empezado.** Reemplaza el ítem 6 de v3 en §7 ("implementar Fase 1/Fase 2 vía `ing/` dentro del Workflow de Genesis") — el trabajo ya no es cablear `ing/` dentro del Workflow existente, sino confirmar/construir el motor genérico de Mandate→Actions→Intents y migrar Genesis a correr sobre él con `domain_baseline: "empty"`. Cierra D-14 de raíz (el hardcode de `"genesis"` deja de tener sentido si no hay Workflow dedicado que lo necesite). |
| **D-25 (nuevo, v3.1)** | Frontera `GenesisTab`/`StandardMandateTab` (Paso 4, §4) — ¿sigue siendo necesaria bajo el motor unificado? | Frontend | **⬜ Abierto, prioridad antes de tocar código del Paso 4.** Hipótesis de trabajo (§0.1, §4): probablemente no — se simplifica a un único `MandateTab` genérico impulsado por estado. Falta confirmación explícita de que no hay un caso de uso visual real que la siga justificando. **Relevante también para D-23/B.4:** define dónde aterriza el botón que dispara la creación "a demanda" desde Core, y dónde se conecta el hook `genesisLaunch.ts` (hoy deliberadamente aislado de `MandateTab`/`tabsStore`, con TODO explícito) — no bloquea lo ya implementado, sí bloquea B.4.2 y el resto de la UI. |
| **D-26 (nuevo, v3.2)** | Reorganización del handler `onboarding:create-mandate` — ¿queda donde está (reusado tal cual desde Core, ya alcanzable hoy sin cambios de Electron) o se mueve a `core/ipc/mandate-handlers.js` bajo un canal propio? | Frontend/Electron | **⬜ Abierto, diferido a propósito.** Confirmado que `core/preload_core.js` ya expone el mismo shape (`createMandate`, `complete`, `markStepComplete`) y que el handler ya está registrado en ambas ramas de `app.whenReady()` — funcionalmente no hace falta mover nada para que Core lo use. La reorganización es una decisión de prolijidad de código, no de arquitectura, y se pospone hasta que D-25 defina la UI final que lo invoca — evita reorganizar dos veces. |
| **D-27 (nuevo, v3.3)** | El step `mandate_genesis` no tiene un único SSOT — está declarado en `onboarding_steps.json` y duplicado en dos `FALLBACK_STEPS` (`milestone-registry.js` y `navigation.js`) | Frontend/Onboarding | **⬜ Abierto, nuevo — hallazgo de la implementación de D-22, no del relevamiento de Fase A.** Los tres quedaron sincronizados manualmente para este cambio, pero el mecanismo sigue siendo frágil: una edición futura que toque un solo lugar deja los otros dos desactualizados sin ningún aviso. No es parte del alcance de D-22/D-23 — se registra como deuda a resolver en una sesión aparte (candidato: que `milestone-registry.js` y `navigation.js` lean de `onboarding_steps.json` en vez de mantener copias propias). |

---

## 7. Qué hacer ahora, en orden de prioridad real (reordenado en v3.1)

**Nota de encuadre:** D-22 y D-23 ya están implementadas (Fase B, Cowork, v3.3) — lo que sigue de esos dos puntos es la QA manual de §1.2, no más código. Sigue sin cerrarse D-25 (frontera de tabs) y D-26 (reorganización del handler, diferida a propósito). D-27 (SSOT triplicado del step) es deuda nueva, sin urgencia.

0. **(v3.3, antes que nada) Correr la QA manual de §1.2** para D-22/D-23 — onboarding completo de punta a punta, cierre/reapertura de Core sin relanzar Genesis, inspección de `nucleus.json` en el medio. Nadie lo corrió todavía.
1. **(v3.1) Cerrar D-25:** confirmar con quien trabaja el roadmap de UI si la frontera `GenesisTab`/`StandardMandateTab` tiene algún caso de uso real que un `MandateTab` genérico con estado no pueda cubrir. Esto define el tamaño real del Paso 4 (§4), y también dónde conectar el hook `genesisLaunch.ts` que hoy quedó deliberadamente aislado.
2. **Confirmar el Paso 2 (tab-bar)** cuando la sesión de Frontend lo entregue — archivo real, no resumen.
3. **(v3.3, sin urgencia) D-27:** evaluar si conviene que `milestone-registry.js` y `navigation.js` dejen de mantener copias propias de `FALLBACK_STEPS` y lean de `onboarding_steps.json` — no bloquea nada de lo implementado, es prevención de drift futuro.
4. **(v3.2, puede diferirse) D-26:** decidir si el handler de creación de mandate se reorganiza a `core/ipc/mandate-handlers.js` o se deja donde está — conviene resolverlo junto con D-25, no antes.
5. **Seguir la secuencia 3 → 4 → 5** de la migración de UI, sin saltos — es la que ya decidimos y la que evita repetir errores de diseñar en abstracto. El Paso 4 arranca del resultado del punto 1 de esta lista, no del planteo original de v3.
6. **Q-02 y Q-08** (los dos endpoints de Backend) pueden ir en paralelo a la migración de UI — no compiten por la misma sesión de Frontend.
7. **Auditoría de Nucleus (Go) para D-21 (switch de organización, G1-G8)** — pedir contenido real de los 5 archivos candidatos más probables: `system_gate.go` (G2, prioridad #1 por nombre), `vault.go` (G5, prioridad alta — es donde una falla de drenado se convierte en fuga de credenciales entre orgs), `ownership.go` (G5/G7), `paths.go` [Nucleus/Go] (G7), `registry.go` (G7) — confirmar o refutar cada guarda contra código real, no contra el mapeo por nombre de archivo del documento de diseño. Recién después, decidir si `system_gate.go` ya es el lugar correcto para G2 o si hace falta un módulo nuevo.
8. **(Reemplaza al ítem 6 de v3 — ver D-24, §2.1) Diseñar y luego implementar el motor genérico de Mandate→Actions→Intents**, confirmando primero si ya existe alguna forma de esto para Mandates `dev/`/`doc/`/etc., y migrar Genesis a correr sobre él (`ing/` con `domain_baseline: "empty"` como primera Action) en vez de seguir invirtiendo en `mandate_genesis_activities.go` como camino especial. Sigue siendo la ruta crítica real para que Capas 1-2 del Bootstrap Strategy dejen de estar bloqueadas por "técnica sin decidir" — priorizar sobre Capa 3, que sigue sin ruta crítica. El mecanismo de invocación CLI subprocess de `brain` (D-15) no cambia, solo cambia quién lo invoca.
8. **Capa 3 del Bootstrap Strategy** (biblioteca de patrones) queda para después del lanzamiento — no es parte de la ruta crítica.
9. **Actualizar `ws-events.ts`** con los 3 eventos faltantes y el payload de `mandate:action:completed` (§5.2) — es deuda chica, cross-cutting, y ya está identificada con precisión de línea de código. Al migrar al motor genérico (punto 7), revisar si los eventos "brandeados" (`mandate:build:validate`, `mandate:build:rejected`, `mandate:build:all_complete`) se generalizan también, o quedan como alias de eventos genéricos de Mandate.
...
- `docs/GOVERNANCE/SWITCH-ORG/ORGANIZATION_SWITCH_ARCHITECTURE.md` — diseño del switch de organización con modelo Single-Org Activa y las 8 guardas (D-21, §6/§7).
