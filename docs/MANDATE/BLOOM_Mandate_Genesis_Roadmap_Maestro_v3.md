...
9. **Actualizar `bloom_project_tree.txt` (nuevo en v3)** para reflejar las rutas de artefactos e índices reales de `ing/` y `dis/`, hoy ausentes o desactualizadas en ese árbol de referencia:
   ...
   - Deuda chica, cross-cutting igual que el punto 8, pero importante para que cualquiera que navegue el filesystem de un Mandate no se encuentre con carpetas `.reception/`/`.discovery/` etc. no documentadas en el árbol de referencia.
