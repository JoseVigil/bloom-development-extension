...
Tratar todo `gravityRules[]` como si fuera enteramente de la primera clase —como hizo, sin decirlo, el catálogo original al describir `resolve_active_gravity` junto al mismo lenguaje de rechazo estructurado que usa `capability_seam`— es precisamente el error de encuadre que hay que dejar de cometer. Cuando se retome el diseño de Gravity, vale la pena preguntar explícitamente, posture por posture: ¿esto es criterio (`verifiable: false`, informa el juicio) o es una restricción dura que de verdad amerita bloquear la firma de un intent (`verifiable: true`)? Son dos mecanismos distintos que hoy comparten un único campo y un único vocabulario, y esa mezcla es la que motivó, con razón, el pedido de dejar de llamarlo "regla".
