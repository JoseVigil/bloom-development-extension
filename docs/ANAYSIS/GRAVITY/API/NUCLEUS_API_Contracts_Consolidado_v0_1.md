...
| `proposer_type` | string (valor confirmado: `"agent"`) | ⚠️ Ambigüedad detectada | **[BTIPS §8.3]** (prosa); **[COR §0, §3.1, §3.3]** | Ver §1.4 — inconsistencia de nombre con el campo `proposed_by` que aparece en los ejemplos JSON de `mandate_state.json` |
...
### 1.2 Variantes por tipo — con los ejemplos de `mandate_state.json` (turnos)

Todos los ejemplos siguientes provienen literalmente de **[BTIPS §8.5]**, el registro de turnos de `mandate_state.json`:
...
### 2.5 Nota de diseño — decisión síncrona vs. registro de turno persistido
...
2. **El registro de turno persistido en `mandate_state.json`** (**[BTIPS §8.5]**, extendido por **[MANDATE v1.2.0 §3]** y **[GRAVITY v0.1 §2.3]**): incluye además `turn`, `proposed_by`, `intent_draft` (copia completa), `result` y, opcionalmente, `note` — campos que sólo tienen sentido **después** de la ejecución (o que documentan explícitamente el rechazo, con `result: null`).

Ninguna de las cuatro fuentes separa formalmente estos dos objetos con nombres propios — el ejemplo de `mandate_state.json` en **[BTIPS §8.5]** muestra el turno completo ya persistido, con la decisión de Nucleus embebida adentro. Este documento hace explícita esa distinción como una decisión de modelado (no un campo nuevo): tratarlos como un único DTO obligaría a que campos como `result`/`note` (que no existen en el momento de responder `validate_and_sign`) aparezcan como opcionales sin motivo aparente para quien sólo mira la respuesta síncrona.
