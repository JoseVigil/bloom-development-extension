...
**Fecha:** 2026-08-29
**Dominio:** Orbital · Gravity · Nucleus · Paladin
...
**No reabre:**
- El gesto de postulación en Paladin (**UX** §1) — este documento asume que el modo de escritura de una Postura ya está activo cuando la gramática se aplica; no diseña cómo se activa ese modo.
...
- **Paladin:** dentro del objeto `PosturaDraft` que **Client** §2 ya define (cliente-only, mutable hasta confirmar) — específicamente sobre el campo `criterio` de ese borrador, mientras el panel inline de **UX** §1.4 está abierto. El resultado del parseo advisory es, en los mismos términos que **Client** usa para clasificar objetos, un dato **cliente-only y efímero**: nunca se envía al backend como tal, se descarta con el resto del `PosturaDraft` si el ingeniero cierra sin confirmar (**Client** §3), y no bloquea por sí mismo el botón "Confirmar postulación" — ese control sigue perteneciendo enteramente a la respuesta de Nucleus, nunca al juicio del cliente. (Cómo se muestra visualmente ese feedback es una decisión de UI que este documento no toma — **UX** ya la dejó fuera de su propio alcance en tokens visuales, §6, y este documento no la reabre tampoco.)

### 3.4 Tecnología — evaluación de opciones

**Evidencia de contexto (marcada explícitamente, no asumida):** el repositorio ya comparte tipos entre componentes (Batcave, Conductor, el backend nuevo de Cloudflare Workers) a través de una carpeta `contracts/` en TypeScript validada con Zod (`Backend_Cloudflare_Arquitectura_v0_1.md` §2–§3, documento del mismo proyecto) — ✅ confirmado para esos componentes. ⚠️ **No confirmado**: que el núcleo de validación de Nucleus (el que firma `gravityRules[]`) corra sobre ese mismo runtime TypeScript — el corpus de Gravity nunca lo dice explícitamente. Este documento evalúa las opciones bajo ambos escenarios.
...
| Riesgo de divergencia cliente/servidor si ambos corren el **mismo** runtime (escenario ✅ probable) | Ninguno — un solo módulo TypeScript, importado literalmente igual por Paladin y por Nucleus, mismo patrón que `contracts/` ya usa hoy para `OwnershipSchema`/`BatcaveConfigSchema` | Ninguno tampoco, pero agrega una etapa de build (compilar `.pegjs`/`.ne` a código) sin beneficio real sobre (A) en este escenario | Ninguno tampoco, pero agrega un toolchain de codegen (Java) para un problema que un solo módulo ya resuelve en este escenario |
| Riesgo de divergencia si los runtimes **difieren** (escenario ⚠️ no descartable) | Alto — exige mantener dos implementaciones escritas a mano en dos lenguajes, exactamente el riesgo que §3.1 señala | Medio — el archivo de gramática se comparte como texto, pero la sintaxis de acciones semánticas de cada generador PEG no es portable entre herramientas de distinto lenguaje; "mismo archivo" no garantiza "mismo parser" | Bajo — un único archivo `.g4` genera parsers para múltiples lenguajes objetivo (incluye TypeScript/JavaScript y Python) desde la misma definición formal; es la herramienta diseñada específicamente para este problema |
...
**3.4.1 Decisión.** Se recomienda **(A) recursive descent a mano, como módulo único**, bajo el escenario ✅ confirmado (Paladin y la superficie de validación de Nucleus comparten runtime TypeScript, siguiendo el patrón `contracts/` ya vigente en el repo) — es la opción que menor superficie nueva introduce, da control total sobre los mensajes de error que §6 exige, y no paga ningún costo de codegen para una gramática de este tamaño. Elegir (B) o (C) en este escenario sería sobredimensionar la solución exactamente en el sentido que **Persistencia** §2.3 ya usó para descartar un motor de grafo dedicado: resolver una capacidad (parseo de gramáticas ambiguas o generación multi-lenguaje) que este problema real no ejercita.

**3.4.2 Condición de fallback, explícita y verificable.** Si se confirma que el punto de validación autoritativo de Nucleus corre sobre un runtime distinto al de Paladin (escenario ⚠️), la recomendación cambia a **(C) ANTLR4**, con el archivo `.g4` como artefacto canónico versionado (mismo tratamiento que cualquier otro contrato compartido del repo) y generación de parser para cada lenguaje objetivo en el mismo paso de build — nunca (B), porque en el escenario multi-runtime (B) da la falsa sensación de "gramática compartida" sin la garantía real que (C) sí ofrece (mismo `.g4`, mismo generador, semántica de parseo idéntica por construcción). Esta condición es mecánicamente verificable antes de implementar: basta confirmar el runtime del punto de firma en Nucleus.
...
La distinción importa porque solo la primera es detectable enteramente en el cliente (Paladin no necesita el Grafo de Gravedad real para saber que `on` quedó sin argumento); la segunda exige, por diseño, la verificación autoritativa de Nucleus contra datos que el cliente nunca ve completos (**API** §3.1, misma justificación de opacidad que ya rige la exposición del grafo completo). Paladin puede dar una verificación *advisory* parcial de la segunda clase cuando el dato ya es visible en el breadcrumb resuelto de ese turno (**UX** §2.3 ya lista los `ruleId` heredados aplicables) — pero nunca la trata como definitiva, mismo principio que §3.2 ya fija para el resto del parseo del lado del cliente.
