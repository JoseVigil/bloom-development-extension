...
**Estado:** v0.1 — introducción y encuadre, sin diseño de algoritmo todavía. Actualizado 2026-09-01 con terminología migrada a Posture y nota de estado de implementación real.
**Fecha:** 2026-09-01
...
> **Nota de terminología (migrada, 2026-09-01):** el rename transversal `GravityRule` → `GravityPosture` (`gravityRules[]`→`gravityPostures[]`, `ruleId`→`postureId`, y derivados) ya está validado e implementado end-to-end en Go/TypeScript, confirmado por Codex. Este documento usa exclusivamente `GravityPosture`/`gravityPostures[]` desde esta versión.
>
> **Nota de estado (2026-09-01):** la misma actualización de Codex confirma, con código real, dos piezas que este documento venía asumiendo por diseño documental: `Store.CreateNode` falla cerrado para nodos `ORGANIZATION`/`NUCLEUS` hasta que estén conectados `cor` y el módulo de Authorization (soporta directamente el invariante "la organización conserva la soberanía para decidir" de la introducción de Jose, §0), y la persistencia filesystem del `GravityGraph` ya existe bajo `.bloom/.nucleus-{organization}/.gravity/`. Ninguna de las cuatro preguntas abiertas de §2 queda resuelta por esto — siguen sin existir el algoritmo de contraste, el vocabulario de resultado ni el contrato de salida hacia Nucleus.
...
- Consecuencia directa para este tema: lo que viaja con un Mandate para comparación **no puede ser el `GravityGraph` completo** — hacerlo rompería la soberanía local que ese mismo cierre acaba de fijar como invariante. Tiene que ser una representación más chica, todavía sin definir, de las posturas/`gravityPostures[]` relevantes del Mandate. Esa definición es, con otras palabras, la primera pregunta que abre la introducción de Jose — el cierre de boundary la confirma desde un ángulo distinto e independiente.
