...
CREATE TABLE authority_project_claims(project_id TEXT PRIMARY KEY NOT NULL,organization_id TEXT NOT NULL REFERENCES organizations(id),tenant_id TEXT NOT NULL REFERENCES tenants(id),installation_id TEXT NOT NULL REFERENCES installation_keys(installation_id),revision TEXT NOT NULL CHECK(revision='1'),source_ref TEXT NOT NULL,evidence_kind TEXT NOT NULL CHECK(evidence_kind='canonical'),claimed_at TEXT NOT NULL,CHECK(source_ref='installation:'||installation_id));
