#!/usr/bin/env bash
# terrabiter_rebuild.sh — BORRA Y RECONSTRUYE el disco de TerraBiter (/dev/sda) para reutilizarlo.
#
...
#   bash terrabiter_rebuild.sh status          # solo lectura: identifica el disco y su estado
#   bash terrabiter_rebuild.sh wipe            # desmonta y escribe ceros en TODO el disco (2-3 h)
#   bash terrabiter_rebuild.sh format ext4     # particiona, formatea, monta en /mnt/terrabiter (o: ntfs)
#   bash terrabiter_rebuild.sh verify          # compara SMART y lanza la prueba larga del disco
#   PERSIST=1 bash terrabiter_rebuild.sh format ext4   # además agrega una línea a /etc/fstab (con copia)
set -uo pipefail
...
LABEL="TerraBiter"
OWNER="jose"
LOGDIR="$HOME/terrabiter_rebuild"
mkdir -p "$LOGDIR"
...
identify() {
  ...
  local other; other=$(lsblk -nro MOUNTPOINTS "$DEV" | grep -v -x -e '' -e "$MNT" || true)
...
release_mount() {
  ...
  if mountpoint -q "$MNT"; then
    echo "procesos usando $MNT:"; sudo fuser -vm "$MNT" 2>&1 | tail -n +2 || true
    sudo umount "$MNT" || die "no se pudo desmontar $MNT (cerrar lo que lo use y reintentar)"
  fi
  mountpoint -q "$MNT" && die "$MNT sigue montado"
  echo "$MNT desmontado"
}
...
cmd_status() {
  ...
  systemctl show mnt-terrabiter.mount -p FragmentPath -p What -p Type -p Options 2>/dev/null
  grep -n "terrabiter\|$(basename "$DEV")" /etc/fstab || echo "(sin entrada en /etc/fstab: el montaje no persiste al reiniciar)"
...
cmd_format() {
  ...
  sec "identificación"; identify
  mountpoint -q "$MNT" && die "$MNT está montado; correr wipe primero"
  confirm
  ...
  sec "formato $fs"
  if [ "$fs" = "ext4" ]; then
    sudo mkfs.ext4 -F -L "$LABEL" -m 0 "$part" || die "mkfs.ext4 falló"
  else
